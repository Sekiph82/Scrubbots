...
| AL-055 | Perimeter target reachability | An exterior-entry planner that seeds only OPEN perimeter cells can miss an assigned ACTIVE target directly reachable from outside because TARGET is not ordinary OPEN transit. | Adversarially test outside-to-perimeter-target final arrival with every non-target perimeter cell blocked; support target as final arrival without opening it as transit. | M17 full-surface re-audit, 2026-09-07 |
| AL-056 | Stateful access object construction | Hardening the routing caller is insufficient if the production access object itself can be constructed with malformed state and later fault. | Validate exact state ownership inside the access object, expose read-only coherence, and make invalid/unbound/non-finite public queries fail closed. | M17 full-surface re-audit, 2026-09-07 |
| AL-057 | Mutable numeric tuning | Public float tuning can carry NaN/INF even when types are declared, poisoning geometry math before final validation. | Define deterministic safe behavior for non-finite/negative geometry tuning and prove no non-finite route can escape. | M17 full-surface re-audit, 2026-09-07 |
...
| AL-058 | Stateful dependency validation must survive time | Validating a duck-typed dependency only at bind time is insufficient if later calls can observe changed/malformed return truth; a failed live sync can otherwise leave stale cache queryable. Null/special failure branches can also bypass the common neutralization path. | Make every bind failure obey one lifecycle policy; distinguish healthy false from malformed return types on live calls; invalidate stale derived cache on dependency-truth corruption; prefer the narrow lifecycle category actually used by production (for M13, RefCounted BoardState-compatible dependencies). | M13-C001 V03 independent audit, 2026-09-07 |

| AL-059 | Cached-domain coherence | A boolean false from a live dependency is not automatically a healthy caller error. If the index was inside the domain successfully cached earlier, a later false can contradict the dependency snapshot and leave stale derived truth queryable. | Persist the validated domain metadata atomically with the cache; classify caller-out-of-domain separately from dependency contradiction; neutralize on contradiction. Also enforce production maximums before iterating duck-typed counts. | M13-C001 V04 independent audit, 2026-09-07 |
