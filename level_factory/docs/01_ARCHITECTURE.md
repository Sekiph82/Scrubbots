...
`data/palettes/scrubbots_palette_v3.json` (root), with Factory-facing
...
- **Palette V3:** root game's owner-locked **C01..C16** is authoritative. Factory
  production output uses only a used subset in ascending global C-ID order. No
  C16, no off-palette RGB.
- **Used-color envelope:** 3..12 distinct canonical logical cell colors actually
  used by artwork cells; `difficultyClassDerivedFromColorCount` is false.
