...
- [ ] SB-M09-018 Batch import. **CHANGES_REQUIRED by ChatGPT audit V02 (F-M09B-006): final destination path itself can still be an existing directory and fail deterministically during commit after an earlier item writes. V03 correction issued.** — M09-C002: `LevelBatchImporter.run_batch()` (`scripts/tools/level_batch_importer.gd`) + CLI `tools/import_level_batch.gd`. Manifest-driven, reuses audited `LevelImporter` per item (`dry_run` param), preserves manifest order. Validation-only and commit (`--commit`) modes both real-CLI-verified with a multi-item batch (3×2 TEST, 20×27 EASY): validation-only writes nothing, commit writes, rerun reports unchanged. V02 correction (audit V01 F-M09B-001/002): destination-parent-directory and catalog-root existence are now part of preflight, not discovered mid-commit — CLI-verified (missing parent / missing / non-directory catalog root all rejected before any write, exit 1). 426/426 tests pass (32 new V02 checks on top of the 394 V01 baseline).
- [ ] SB-M09-019 Batch validation. **CHANGES_REQUIRED by ChatGPT audit V02 (F-M09B-006): destination-object type must be preflighted before any commit.** — Whole-batch preflight (every item `dry_run=true` + schema/destination-parent/duplicate-ID/cross-item-path/catalog-ownership/catalog-health checks) runs before any commit write; a failing later item blocks earlier items from being committed (CLI-verified, including the missing-destination-parent case). Malformed/empty manifest, missing required fields, and wrong optional-field types (V02 correction, audit V01 F-M09B-005) rejected with actionable per-item errors and no crash. V02 correction (audit V01 F-M09B-004): existing catalog malformed/duplicate-ID corruption now invalidates the whole batch's `is_ok()` even when unrelated to the requested items, not merely reported informationally — CLI-verified (exit 1).
