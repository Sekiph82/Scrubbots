...
func _initialize() -> void:
	...
	_run_m20_v07_consumer_claim_tests()
	_run_m20_v07_agent_parent_tests()
	_run_m20_v07_renderer_variant_tests()
	_run_m20_v07_desync_activation_tests()
	_run_m20_v07_sb_ledger_tests()
	# M20-C001 V08 — final whole-sprint validation-only gate (fresh arrangements).
	_run_m20_v08_auditor_validation_tests()
	# M20-C001 V09 — final exact-evidence validation-only gate.
...
# synchronized cross-module clear transaction in the accepted order
# (BoardState -> candidate index -> reservation -> dispatcher finalize -> renderer,
# renderer presentation-only and post-finalize). Uses REAL production selection/
...
# ============================ M20-C001 V07 batched whole-sprint correction =====
# Two production corrections + whole-sprint SB-M20 evidence:
#   A (F-M20-STRICT-001.M): one M20 arrival-consumer per dispatcher (weak claim).
#   B (F-M20-STRICT-001.N): explicit agent_parent presence via Variant type.
# Test-only harnesses stay for adversarial candidate/reservation subclasses; the
# production exact-category gates are never widened.

## Fresh unbound real dispatcher + its M19 deps over `board` (no agent_parent yet).
func _m19_parts(board) -> Dictionary:
	var reservations = ReservationState.new(); reservations.bind(board)
	var candidates = ColorCandidateIndex.create(); candidates.bind(board)
	var selector = TargetSelector.create(); selector.bind(board, candidates, reservations)
	var routing = ProductionRoutingSystem.new()
...
func _m19_parts_teardown(p: Dictionary) -> void:
	p["d"].reset(); root.remove_child(p["d"]); p["d"].free()

# --- §2 correction A: single M20 arrival consumer per dispatcher ---------------
func _run_m20_v07_consumer_claim_tests() -> void:
	print("---- M20-C001 V07 §2: single M20 arrival consumer per dispatcher (F-M20-STRICT-001.M) ----")
	var board = _m19_open_board_active(20, 20, [Vector2(10, 10)])
	var color: int = board.get_color_id(board.get_cell_index(10, 10))
	var w = _m20_full(board)
	var slots = _m20_slots([color, color, color, color, color])
	...
	_check(l1.bind(board, slots, w["candidates"], w["reservations"], w["dispatcher"]), "claim: first healthy loop binds")
	# A benign diagnostic listener must NOT block the M20 owner claim.
	var diag := func(_o, _t, _c, _a): pass
	w["dispatcher"].assignment_arrived.connect(diag)
	var conn_before: int = _m20_conn_count(w["dispatcher"])
	var l2 = CompleteClearingLoop.new()
	_check(not l2.bind(board, _m20_slots([color, color, color, color, color]), w["candidates"], w["reservations"], w["dispatcher"]), "claim: second different loop rejected (owner claimed)")
	_check(not l2.is_bound(), "claim: second loop unbound")
	_check_eq(_m20_conn_count(w["dispatcher"]), conn_before, "claim: rejected second loop adds no arrival connection")
	# First loop remains coherent/usable; second cannot mutate canonical state.
	_check(l1.is_coherent(), "claim: first loop still coherent")
	_check(not l2.activate_slot(0, Vector2(-2.0, 10.5)).success, "claim: unbound second loop cannot activate")
	var r1 = _m20_activate_and_arrive(l1, 0, Vector2(-2.0, 10.5))
	_check(r1.success and l1.get_cleared_count() == 1, "claim: first loop clears normally")
	# Freeing the first loop releases the claim -> a fresh loop can bind the dispatcher.
	l1 = null
	var b2 = _m19_open_board_active(20, 20, [Vector2(12, 10)])
	# (new active cell so there is work; reuse same dispatcher bundle board is `board`,
	#  so use board's remaining cell) -> simpler: prove a fresh loop now binds.
	var l3 = CompleteClearingLoop.new()
	_check(l3.bind(board, _m20_slots([color, color, color, color, color]), w["candidates"], w["reservations"], w["dispatcher"]), "claim: released after first loop freed -> fresh loop binds")
	l3 = null
	w["dispatcher"].assignment_arrived.disconnect(diag)
	_m20_teardown(w)

# --- §3 correction B: explicit agent_parent presence law -----------------------
func _run_m20_v07_agent_parent_tests() -> void:
	print("---- M20-C001 V07 §3: explicit agent_parent presence law (F-M20-STRICT-001.N) ----")
	# Omitted parent -> dispatcher self; spawned agent attaches under the dispatcher.
	var b1 = _m19_open_board_active(20, 20, [Vector2(10, 10)])
	var c1: int = b1.get_color_id(b1.get_cell_index(10, 10))
	var p1 = _m19_parts(b1)
	_check(p1["d"].bind(b1, p1["selector"], p1["reservations"], p1["routing"], p1["ra"], p1["sa"]), "parent: omitted -> bind true (self)")
	var r1 = p1["d"].dispatch(c1, Vector2(-2.0, 10.5), 6.0)
	_check(r1.success and r1.agent.get_parent() == p1["d"], "parent: omitted -> agent attaches under dispatcher self")
	_m19_parts_teardown(p1)

	# Explicit null -> same legitimate self semantics.
	var b2 = _m19_open_board_active(20, 20, [Vector2(10, 10)])
	var c2: int = b2.get_color_id(b2.get_cell_index(10, 10))
	var p2 = _m19_parts(b2)
	_check(p2["d"].bind(b2, p2["selector"], p2["reservations"], p2["routing"], p2["ra"], p2["sa"], null), "parent: explicit null -> bind true (self)")
	var r2 = p2["d"].dispatch(c2, Vector2(-2.0, 10.5), 6.0)
	_check(r2.success and r2.agent.get_parent() == p2["d"], "parent: explicit null -> agent under self")
	_m19_parts_teardown(p2)

	# Healthy explicit Node parent -> bind true; spawned agent attaches THERE.
	var b3 = _m19_open_board_active(20, 20, [Vector2(10, 10)])
	var c3: int = b3.get_color_id(b3.get_cell_index(10, 10))
	var p3 = _m19_parts(b3)
	var parent = Node.new(); root.add_child(parent)
	_check(p3["d"].bind(b3, p3["selector"], p3["reservations"], p3["routing"], p3["ra"], p3["sa"], parent), "parent: healthy explicit Node -> bind true")
	var r3 = p3["d"].dispatch(c3, Vector2(-2.0, 10.5), 6.0)
	_check(r3.success and r3.agent.get_parent() == parent, "parent: agent attaches under the explicit parent")
	_m19_parts_teardown(p3)
	root.remove_child(parent); parent.free()

	# Scalar explicit parent -> bind false, no SCRIPT ERROR (untyped param).
	var b4 = _m19_open_board_active(20, 20, [Vector2(10, 10)])
	var p4 = _m19_parts(b4)
	_check(not p4["d"].bind(b4, p4["selector"], p4["reservations"], p4["routing"], p4["ra"], p4["sa"], 123), "parent: scalar explicit parent rejected")
	_check(not p4["d"].is_bound(), "parent: unbound after scalar parent")
	_m19_parts_teardown(p4)

	# Arbitrary RefCounted (non-Node) explicit parent -> rejected.
	var b5 = _m19_open_board_active(20, 20, [Vector2(10, 10)])
	var p5 = _m19_parts(b5)
	_check(not p5["d"].bind(b5, p5["selector"], p5["reservations"], p5["routing"], p5["ra"], p5["sa"], RefCounted.new()), "parent: RefCounted explicit parent rejected")
	_m19_parts_teardown(p5)

	# Queued Node explicit parent -> rejected.
	var b6 = _m19_open_board_active(20, 20, [Vector2(10, 10)])
	var p6 = _m19_parts(b6)
	var qparent = Node.new(); root.add_child(qparent); qparent.queue_free()
	_check(not p6["d"].bind(b6, p6["selector"], p6["reservations"], p6["routing"], p6["ra"], p6["sa"], qparent), "parent: queued explicit parent rejected")
	root.remove_child(qparent); qparent.free()
	_m19_parts_teardown(p6)
	# Truly-freed explicit parent -> tests/m20_v07_lifecycle_smoke.gd (needs frames).

# --- §4 renderer variant / lifecycle evidence ----------------------------------
func _run_m20_v07_renderer_variant_tests() -> void:
	print("---- M20-C001 V07 §4: renderer variant / whole-lifecycle evidence ----")
	var board = _m19_open_board_active(20, 20, [Vector2(10, 10)])
	var color: int = board.get_color_id(board.get_cell_index(10, 10))
	var w = _m20_full(board)
	var invalid: Array = [123, 1.5, "x", Vector2(1, 2), [1, 2], {"a": 1}, RefCounted.new(), w["dispatcher"]]
	var names: Array = ["int", "float", "String", "Vector2", "Array", "Dictionary", "RefCounted", "wrong Node"]
	for i in invalid.size():
		var lo = CompleteClearingLoop.new()
		_check(not lo.bind(board, _m20_slots([color, color, color, color, color]), w["candidates"], w["reservations"], w["dispatcher"], invalid[i]), "renderer-variant: %s rejected" % names[i])
		_check(not lo.is_bound(), "renderer-variant: %s leaves loop unbound" % names[i])
	_check_eq(_m20_conn_count(w["dispatcher"]), 0, "renderer-variant: no invalid variant connected an arrival signal")
	_m20_teardown(w)

	# Failed-bind metadata recovery on the SAME loop instance.
	var bR = _m19_open_board_active(20, 20, [Vector2(10, 10)])
	var cR: int = bR.get_color_id(bR.get_cell_index(10, 10))
	var wR = _m20_full(bR)
	var loop = CompleteClearingLoop.new()
	_check(not loop.bind(bR, _m20_slots([cR, cR, cR, cR, cR]), wR["candidates"], wR["reservations"], wR["dispatcher"], 999), "renderer-recovery: invalid explicit renderer fails")
	_check(loop.bind(bR, _m20_slots([cR, cR, cR, cR, cR]), wR["candidates"], wR["reservations"], wR["dispatcher"]), "renderer-recovery: same loop then binds clean headless")
	_check(loop.is_coherent(), "renderer-recovery: no stale renderer metadata (headless coherent)")
	_check(_m20_activate_and_arrive(loop, 0, Vector2(-2.0, 10.5)).success and loop.get_cleared_count() == 1, "renderer-recovery: headless clears (no stale presence)")
	loop = null
	_m20_teardown(wR)

	# Second-bind preservation: original configured renderer stays required.
	var bS = _m19_open_board_active(20, 20, [Vector2(10, 10)])
	var cS: int = bS.get_color_id(bS.get_cell_index(10, 10))
	var wS = _m20_full(bS, null, null, true)
	var lS = _m20_loop(wS, _m20_slots([cS, cS, cS, cS, cS]))
	_check(not lS.bind(bS, _m20_slots([cS, cS, cS, cS, cS]), wS["candidates"], wS["reservations"], wS["dispatcher"], null), "renderer-2ndbind: second bind refused")
	# Prove the original renderer is still required: make it foreign-board -> incoherent.
	var otherBoard = _m19_open_board_active(20, 20, [Vector2(3, 3)])
	wS["renderer"].configure(otherBoard, PackedStringArray(BoardDebugFixturesM20.PALETTE), Vector2(120, 120))
	_check(not lS.is_coherent(), "renderer-2ndbind: original configured renderer still required (foreign-board -> incoherent)")
	lS = null
	root.remove_child(wS["renderer"]); wS["renderer"].free(); wS["renderer"] = null
	_m20_teardown(wS)

	# Post-bind drift: reconfigure to a same-size different board -> incoherent +
	# activation fails before any new dispatcher work.
	var bD = _m19_open_board_active(20, 20, [Vector2(10, 10)])
	var cD: int = bD.get_color_id(bD.get_cell_index(10, 10))
	var wD = _m20_full(bD, null, null, true)
	var lD = _m20_loop(wD, _m20_slots([cD, cD, cD, cD, cD]))
	var driftBoard = _m19_open_board_active(20, 20, [Vector2(4, 4)])
	wD["renderer"].configure(driftBoard, PackedStringArray(BoardDebugFixturesM20.PALETTE), Vector2(120, 120))
	_check(not lD.is_coherent(), "renderer-drift: same-size different board -> incoherent")
	_check(not lD.activate_slot(0, Vector2(-2.0, 10.5)).success, "renderer-drift: activation fails before new dispatcher work")
	_check_eq(wD["dispatcher"].get_active_count(), 0, "renderer-drift: no dispatcher work started")
	lD = null
	root.remove_child(wD["renderer"]); wD["renderer"].free(); wD["renderer"] = null
	_m20_teardown(wD)

	# Configured renderer queued AFTER a live assignment, BEFORE arrival: arrival
	# preflight clears/finalizes nothing; reservation+dispatcher stay held until reset.
	var bQ = _m19_open_board_active(20, 20, [Vector2(10, 10)])
	var tQ: int = bQ.get_cell_index(10, 10)
	var cQ: int = bQ.get_color_id(tQ)
	var wQ = _m20_full(bQ, null, null, true)
	var lQ = _m20_loop(wQ, _m20_slots([cQ, cQ, cQ, cQ, cQ]))
	var rQ = lQ.activate_slot(0, Vector2(-2.0, 10.5), 6.0)
	wQ["renderer"].queue_free() # queued (still non-null) before arrival
	_m19_drive_to_arrival(rQ.agent)
	_check_eq(bQ.get_cell_state(tQ), BoardState.CellState.ACTIVE, "renderer-death: queued renderer before arrival -> no clear")
	_check_eq(lQ.get_cleared_count(), 0, "renderer-death: cleared_count unchanged")
	_check_eq(wQ["reservations"].get_owner(tQ), rQ.owner_id, "renderer-death: reservation held")
	_check(wQ["dispatcher"].has_owner(rQ.owner_id), "renderer-death: dispatcher assignment held until reset")
	lQ.reset()
	_check_eq(wQ["dispatcher"].get_active_count(), 0, "renderer-death: reset cleans the assignment")
	if is_instance_valid(wQ["renderer"]):
		root.remove_child(wQ["renderer"]); wQ["renderer"].free()
	wQ["renderer"] = null
	lQ = null
	_m20_teardown(wQ)

	# Healthy presentation: target alpha 0 after finalize; an unrelated pixel unchanged.
	var bH = _m19_open_board_active(20, 20, [Vector2(10, 10), Vector2(15, 12)])
	var tH: int = bH.get_cell_index(10, 10)
	var cH: int = bH.get_color_id(tH)
	var uH: int = bH.get_cell_index(15, 12) # unrelated ACTIVE cell (different row/color)
	var wH = _m20_full(bH, null, null, true)
	var lH = _m20_loop(wH, _m20_slots([cH, cH, cH, cH, cH]))
	var pos_u: Vector2i = bH.get_cell_position(uH)
	var px_u_before: Color = wH["renderer"].get_pixel_color(pos_u.x, pos_u.y)
	_check(_m20_activate_and_arrive(lH, 0, Vector2(-2.0, 10.5)).success, "renderer-present: healthy clear dispatched")
	var pos_t: Vector2i = bH.get_cell_position(tH)
	_check(_colors_close(wH["renderer"].get_pixel_color(pos_t.x, pos_t.y), Color(0, 0, 0, 0), 0.02), "renderer-present: target alpha 0 after finalize")
	_check_eq(wH["renderer"].get_pixel_color(pos_u.x, pos_u.y), px_u_before, "renderer-present: unrelated pixel unchanged by one-cell repaint")
	lH = null; _m20_teardown(wH)

# --- §5 desync + §6 activation matrices (fresh, compact) -----------------------
func _run_m20_v07_desync_activation_tests() -> void:
	print("---- M20-C001 V07 §5/§6: arrival-desync + activation public-boundary matrix ----")
	# Activation boundary matrix.
	var board = _m19_open_board_active(20, 20, [Vector2(10, 10)])
	var color: int = board.get_color_id(board.get_cell_index(10, 10))
	var w = _m20_full(board)
	var loop = _m20_loop(w, _m20_slots([color, color, color, color, color]))
	var bad_origins: Array = [Vector2(NAN, 10.5), Vector2(INF, 10.5), Vector2(-INF, 10.5), Vector2(10.5, NAN)]
	for o in bad_origins:
		_check(not loop.activate_slot(0, o).success, "activation: non-finite origin %s rejected" % str(o))
	var bad_speeds: Array = [NAN, INF, -INF, 0.0, -1.0]
	for s in bad_speeds:
		_check(not loop.activate_slot(0, Vector2(-2.0, 10.5), s).success, "activation: bad speed %s rejected" % str(s))
	_check(not loop.activate_slot(-1, Vector2(-2.0, 10.5)).success, "activation: slot -1 rejected")
	_check(not loop.activate_slot(5, Vector2(-2.0, 10.5)).success, "activation: slot 5 rejected")
	_check(not loop.activate_slot(1.5, Vector2(-2.0, 10.5)).success, "activation: non-int slot rejected")
	var slots = w.get("slots")
	loop.reset()
	# Unavailable slot.
	var slotsys = _m20_slots([color, color, color, color, color]); slotsys.set_slot_available(0, false)
	var w2 = _m20_full(_m19_open_board_active(20, 20, [Vector2(10, 10)]))
	var l2 = _m20_loop(w2, slotsys)
	_check(not l2.activate_slot(0, Vector2(-2.0, 10.5)).success, "activation: unavailable slot rejected")
	_check_eq(w2["dispatcher"].get_active_count(), 0, "activation: rejected activation spawns no agent")
	_check_eq(w2["reservations"].get_reservation_count(), 0, "activation: rejected activation reserves nothing")
	l2 = null; _m20_teardown(w2)
	# Every rejected activation on `loop` left board unmutated.
	_check_eq(board.count_cells_by_state(BoardState.CellState.ACTIVE), 1, "activation: rejected activations mutated no BoardState cell")
	loop = null; _m20_teardown(w)

	# Arrival desync matrix (representative fresh cases through the M20 boundary).
	# wrong owner/target/color/agent + unknown owner via a real arrived assignment.
	var bd = _m19_open_board_active(20, 20, [Vector2(10, 10)])
	var td: int = bd.get_cell_index(10, 10)
	var cd: int = bd.get_color_id(td)
	var wd = _m20_full(bd)
	var ld = _m20_loop(wd, _m20_slots([cd, cd, cd, cd, cd]))
	var rd = ld.activate_slot(0, Vector2(-2.0, 10.5), 6.0)
	wd["dispatcher"].assignment_arrived.disconnect(ld._arrival_cb)
	_m19_drive_to_arrival(rd.agent)
	var owner: int = rd.owner_id; var agent = rd.agent
	for variant in [[owner + 1, td, cd, agent, "wrong owner"], [owner, td + 1, cd, agent, "wrong target"],
			[owner, td, cd + 1, agent, "wrong color"], [owner, td, cd, null, "wrong agent"], [999, td, cd, agent, "unknown owner"]]:
		var before = _snapshot_cell_states(bd)
		ld._on_assignment_arrived(variant[0], variant[1], variant[2], variant[3])
		_check_eq(ld.get_last_outcome(), CompleteClearingLoop.Outcome.PREFLIGHT_REJECTED, "desync: %s -> PREFLIGHT_REJECTED" % variant[4])
		_check(_cell_states_equal(bd, before), "desync: %s -> no BoardState mutation" % variant[4])
	_check_eq(ld.get_cleared_count(), 0, "desync: no clear across all variants")
	_check(wd["dispatcher"].has_owner(owner), "desync: dispatcher assignment not silently finalized")
	# Missing reservation desync.
	wd["reservations"].release_for_owner(owner)
	ld._on_assignment_arrived(owner, td, cd, agent)
	_check_eq(ld.get_last_outcome(), CompleteClearingLoop.Outcome.PREFLIGHT_REJECTED, "desync: missing reservation -> rejected")
	_check_eq(bd.get_cell_state(td), BoardState.CellState.ACTIVE, "desync: missing reservation -> target ACTIVE")
	wd["dispatcher"].assignment_arrived.connect(ld._arrival_cb)
	ld.reset()
	ld._on_assignment_arrived(owner, td, cd, agent)
	_check_eq(ld.get_cleared_count(), 0, "desync: stale replay after reset cannot clear")
	ld = null; _m20_teardown(wd)

# --- §7 whole-sprint SB-M20-001..014 ledger ------------------------------------
func _run_m20_v07_sb_ledger_tests() -> void:
	print("==== M20-C001 V07 §7: SB-M20-001..014 whole-sprint ledger ====")
	# SB-M20-001 full healthy sequence + exact post-arrival tuple.
	var b1 = _m19_open_board_active(24, 24, [Vector2(11, 11)])
	var t1: int = b1.get_cell_index(11, 11)
	var c1: int = b1.get_color_id(t1)
	var w1 = _m20_full(b1, null, null, true)
	var l1 = _m20_loop(w1, _m20_slots([c1, c1, c1, c1, c1]))
	var r1 = _m20_activate_and_arrive(l1, 0, Vector2(-2.0, 11.5))
	var p1: Vector2i = b1.get_cell_position(t1)
	_check(r1.success, "SB-M20-001: healthy sequence dispatched+arrived")
	_check_eq(b1.get_cell_state(t1), BoardState.CellState.CLEARED, "SB-M20-001: target CLEARED")
	_check(not w1["candidates"].get_candidates(c1, null).has(t1), "SB-M20-001: target absent from raw candidate bucket")
	_check_eq(w1["routing_access"].classify_cell(p1.x, p1.y, -1), ProductionAccessQuery.CellClass.OPEN, "SB-M20-001: ProductionAccessQuery OPEN")
	_check_eq(w1["reservations"].get_owner(t1), -1, "SB-M20-001: reservation owner absent")
	_check(not w1["dispatcher"].has_owner(r1.owner_id), "SB-M20-001: dispatcher owner absent")
	_check(is_instance_valid(r1.agent) and r1.agent.is_queued_for_deletion(), "SB-M20-001: agent queued for deletion")
	_check(_colors_close(w1["renderer"].get_pixel_color(p1.x, p1.y), Color(0, 0, 0, 0), 0.02), "SB-M20-001: renderer alpha 0")
	l1 = null; _m20_teardown(w1)

	# SB-M20-002 no target AND enclosed matching target -> no bot/reservation/clear.
	var b2 = _m19_open_board_active(20, 20, [Vector2(10, 2)])
	var w2 = _m20_full(b2)
	var missing: int = (b2.get_color_id(b2.get_cell_index(10, 2)) + 1) % 5
	var l2 = _m20_loop(w2, _m20_slots([missing, missing, missing, missing, missing]))
	_check_eq(l2.activate_slot(0, Vector2(-2.0, 2.5)).failure_reason, DispatchResult.FailureReason.NO_REACHABLE_TARGET, "SB-M20-002: no target -> NO_REACHABLE_TARGET")
	_check_eq(w2["dispatcher"].get_active_count(), 0, "SB-M20-002: no bot spawned")
	l2 = null; _m20_teardown(w2)
	var b2e = _m19_enclosed_sole_candidate_board()
	var w2e = _m20_full(b2e)
	var l2e = _m20_loop(w2e, _m20_slots([1, 1, 1, 1, 1]))
	_check_eq(l2e.activate_slot(0, Vector2(-2.0, 2.5)).failure_reason, DispatchResult.FailureReason.NO_REACHABLE_TARGET, "SB-M20-002: enclosed candidate -> no clear")
	l2e = null; _m20_teardown(w2e)

	# SB-M20-003 frame-aware queue_free proof -> tests/m20_queue_free_smoke.gd
	print("  SB-M20-003: frame-aware deferred destruction proven by tests/m20_queue_free_smoke.gd")

	# SB-M20-004 true 1x1 clear + exhaustion.
	var b4 = _m19_open_board_active(1, 1, [Vector2(0, 0)])
	_check(b4.get_cell_count() == 1, "SB-M20-004: true 1x1 board")
	var c4: int = b4.get_color_id(0)
	var w4 = _m20_full(b4)
	var l4 = _m20_loop(w4, _m20_slots([c4, c4, c4, c4, c4]))
	_check(_m20_activate_and_arrive(l4, 0, Vector2(-2.0, 0.5)).success and l4.get_cleared_count() == 1, "SB-M20-004: 1x1 clears once")
	_check_eq(l4.activate_slot(0, Vector2(-2.0, 0.5)).failure_reason, DispatchResult.FailureReason.NO_REACHABLE_TARGET, "SB-M20-004: exhausted")
	l4 = null; _m20_teardown(w4)

	# SB-M20-005 one-color exhaustion.
	var cells5: Array = []
	for x in range(6):
		cells5.append(Vector2(x + 2, 6))
	var b5 = _m19_open_board_active(20, 20, cells5)
	var c5: int = b5.get_color_id(b5.get_cell_index(2, 6))
	var w5 = _m20_full(b5)
	var l5 = _m20_loop(w5, _m20_slots([c5, c5, c5, c5, c5]))
	var n5 := 0
	for _i in range(20):
		if not _m20_activate_and_arrive(l5, 0, Vector2(-2.0, 6.5)).success:
			break
		n5 += 1
	_check(n5 == 6 and b5.count_cells_by_state(BoardState.CellState.ACTIVE) == 0, "SB-M20-005: one-color exhausted (6 cleared)")
	l5 = null; _m20_teardown(w5)

	# SB-M20-006 multi-color via corresponding slots, no cross-color corruption.
	var b6 = _m19_open_board_active(20, 20, [Vector2(10, 0), Vector2(10, 1), Vector2(10, 2)])
	var pal6: Array = [b6.get_color_id(b6.get_cell_index(10, 0)), b6.get_color_id(b6.get_cell_index(10, 1)), b6.get_color_id(b6.get_cell_index(10, 2)), 0, 0]
	var w6 = _m20_full(b6)
	var l6 = _m20_loop(w6, _m20_slots(pal6))
	_check(_m20_activate_and_arrive(l6, 0, Vector2(-2.0, 0.5)).target_index == b6.get_cell_index(10, 0), "SB-M20-006: slot0 clears its color")
	_check(_m20_activate_and_arrive(l6, 1, Vector2(-2.0, 1.5)).target_index == b6.get_cell_index(10, 1), "SB-M20-006: slot1 clears its color")
	_check(_m20_activate_and_arrive(l6, 2, Vector2(-2.0, 2.5)).target_index == b6.get_cell_index(10, 2), "SB-M20-006: slot2 clears its color")
	_check_eq(l6.get_cleared_count(), 3, "SB-M20-006: three multi-color clears, no corruption")
	l6 = null; _m20_teardown(w6)

	# SB-M20-007 five slots, five in-flight, unique owners/targets/pairs; first preserves four.
	var b7 = _m19_open_board_active(24, 24, [Vector2(11, 0), Vector2(11, 1), Vector2(11, 2), Vector2(11, 3), Vector2(11, 4)])
	var pal7: Array = []
	for y in range(5):
		pal7.append(b7.get_color_id(b7.get_cell_index(11, y)))
	var w7 = _m20_full(b7)
	var l7 = _m20_loop(w7, _m20_slots(pal7))
	var origins7: Array = [Vector2(-2.0, 0.5), Vector2(26.0, 1.5), Vector2(-2.0, 2.5), Vector2(26.0, 3.5), Vector2(-2.0, 4.5)]
	var res7: Array = []; var ow := {}; var tg := {}
	for i in range(5):
		var r = l7.activate_slot(i, origins7[i], 6.0)
		if r.success:
			res7.append(r); ow[r.owner_id] = true; tg[r.target_index] = true
	_check(res7.size() == 5 and ow.size() == 5 and tg.size() == 5, "SB-M20-007: five in-flight, unique owners+targets")
	var pairs7 := true
	for r in res7:
		if w7["reservations"].get_owner(r.target_index) != r.owner_id:
			pairs7 = false
	_check(pairs7, "SB-M20-007: five exact reservation pairs")
	_m19_drive_to_arrival(res7[0].agent)
	var four7 := true
	for i in range(1, 5):
		if not w7["dispatcher"].has_owner(res7[i].owner_id):
			four7 = false
	_check(four7, "SB-M20-007: resolving one preserves the other four")
	for i in range(1, 5):
		_m19_drive_to_arrival(res7[i].agent)
	_check_eq(l7.get_cleared_count(), 5, "SB-M20-007: all five arrive/resolve")
	l7 = null; _m20_teardown(w7)

	# SB-M20-008..011 Easy/Medium/Hard/Very-Hard representative dims.
	var dims: Array = [{"w": 24, "h": 24, "n": "008 Easy"}, {"w": 34, "h": 34, "n": "009 Medium"},
		{"w": 44, "h": 44, "n": "010 Hard"}, {"w": 54, "h": 54, "n": "011 Very Hard"}]
	for d in dims:
		var bd = _m19_open_board_active(d["w"], d["h"], [Vector2(12, 10)])
		var cd: int = bd.get_color_id(bd.get_cell_index(12, 10))
		var wd = _m20_full(bd)
		var ld = _m20_loop(wd, _m20_slots([cd, cd, cd, cd, cd]))
		_check(_m20_activate_and_arrive(ld, 0, Vector2(-2.0, 10.5)).success and ld.get_cleared_count() == 1, "SB-M20-%s: clears" % d["n"])
		ld = null; _m20_teardown(wd)

	# SB-M20-012 59x59 single-cell path, exactly one cell changed.
	var big = _m19_open_board_active(59, 59, [Vector2(30, 30)])
	var cbig: int = big.get_color_id(big.get_cell_index(30, 30))
	var wbig = _m20_full(big)
	var lbig = _m20_loop(wbig, _m20_slots([cbig, cbig, cbig, cbig, cbig]))
	var before_big: int = big.count_cells_by_state(BoardState.CellState.CLEARED)
	_check(_m20_activate_and_arrive(lbig, 0, Vector2(-2.0, 30.5)).success, "SB-M20-012: 59x59 clears")
	_check_eq(big.count_cells_by_state(BoardState.CellState.CLEARED), before_big + 1, "SB-M20-012: exactly one cell changed (single-cell path)")
	lbig = null; _m20_teardown(wbig)

	# SB-M20-013 rectangular production board.
	var rect = _m19_open_board_active(53, 59, [Vector2(20, 30)])
	var crect: int = rect.get_color_id(rect.get_cell_index(20, 30))
	var wrect = _m20_full(rect)
	var lrect = _m20_loop(wrect, _m20_slots([crect, crect, crect, crect, crect]))
	_check(_m20_activate_and_arrive(lrect, 0, Vector2(-2.0, 30.5)).success and lrect.get_cleared_count() == 1, "SB-M20-013: rectangular 53x59 clears")
	lrect = null; _m20_teardown(wrect)

	# SB-M20-014 AL-028 second-B + rapid >=25 (desync/rollback covered above + V05).
	var bA = _m19_open_board_active(8, 8, [Vector2(1, 2), Vector2(2, 2), Vector2(3, 2), Vector2(2, 1), Vector2(2, 3)])
	var door: int = bA.get_cell_index(1, 2); var inner: int = bA.get_cell_index(2, 2)
	var cA: int = bA.get_color_id(door)
	var wA = _m20_full(bA)
	var lA = _m20_loop(wA, _m20_slots([cA, cA, cA, cA, cA]))
	_check(_m20_activate_and_arrive(lA, 0, Vector2(-2.0, 2.5)).target_index == door, "SB-M20-014: AL-028 gate A cleared")
	var rB = lA.activate_slot(1, Vector2(-2.0, 2.5), 6.0)
	_check(rB.success and rB.target_index == inner, "SB-M20-014: second real activation selects B")
	_m19_drive_to_arrival(rB.agent)
	_check_eq(bA.get_cell_state(inner), BoardState.CellState.CLEARED, "SB-M20-014: B cleared")
	lA = null; _m20_teardown(wA)
	var rc: Array = []
	for x in range(28):
		rc.append(Vector2(x + 1, 6))
	var brap = _m19_open_board_active(30, 12, rc)
	var crap: int = brap.get_color_id(brap.get_cell_index(1, 6))
	var wrap = _m20_full(brap)
	var lrap = _m20_loop(wrap, _m20_slots([crap, crap, crap, crap, crap]))
	var cyc := 0
	for _i in range(40):
		if not _m20_activate_and_arrive(lrap, _i % 5, Vector2(-2.0, 6.5)).success:
			break
		cyc += 1
	_check(cyc >= 25 and lrap.get_cleared_count() == 28, "SB-M20-014: rapid >=25 sequential cycles (%d)" % cyc)
	lrap = null; _m20_teardown(wrap)

# =========================== M20-C001 V08 final whole-sprint validation gate ====
# Fresh, independent adversarial arrangements against the LOCKED V07 production
# (blobs 06391839.../eee10149...). Validation-only: no production change. Frame-only
# cases (GC claim release, truly-freed parent/renderer) live in
# tests/m20_v08_lifecycle_smoke.gd.

func _run_m20_v08_auditor_validation_tests() -> void:
	print("==== M20-C001 V08 auditor validation (fresh arrangements) ====")
	_v08_owner_claim()
	_v08_agent_parent()
	_v08_renderer()
	_v08_desync()
	...
	_v08_ledger()
	_v08_rollback()
	print("  M20 V08 auditor validation complete")

# --- §3 single M20 owner / claim lifecycle -----------------------------------
func _v08_owner_claim() -> void:
	print("---- V08 §3: single M20 owner / claim lifecycle ----")
	var board = _m19_open_board_active(22, 22, [Vector2(11, 11)])
	var color: int = board.get_color_id(board.get_cell_index(11, 11))
	var w = _m20_full(board)
	var l1 = CompleteClearingLoop.new()
	_check(l1.bind(board, _m20_slots([color, color, color, color, color]), w["candidates"], w["reservations"], w["dispatcher"]), "V08.3: first loop binds")
	var diag := func(_o, _t, _c, _a): pass
	w["dispatcher"].assignment_arrived.connect(diag)
	var conn0: int = _m20_conn_count(w["dispatcher"])
	var l2 = CompleteClearingLoop.new()
	_check(not l2.bind(board, _m20_slots([color, color, color, color, color]), w["candidates"], w["reservations"], w["dispatcher"]), "V08.3: second live loop rejected")
	_check_eq(_m20_conn_count(w["dispatcher"]), conn0, "V08.3: rejected second loop adds no connection")
	_check(l1.is_coherent(), "V08.3: first loop still coherent")
	_check(not l2.activate_slot(0, Vector2(-2.0, 11.5)).success, "V08.3: unbound second loop cannot activate")
	l2.reset() # unbound loop reset is a no-op on canonical state
	_check_eq(w["dispatcher"].get_active_count(), 0, "V08.3: unbound second loop reset touches nothing")
	var r1 = _m20_activate_and_arrive(l1, 0, Vector2(-2.0, 11.5))
	_check(r1.success and l1.get_cleared_count() == 1, "V08.3: first loop gets exactly one clear")
	# Owner loop reset() while still ALIVE does NOT release the claim.
	l1.reset()
	var l3 = CompleteClearingLoop.new()
	_check(not l3.bind(board, _m20_slots([color, color, color, color, color]), w["candidates"], w["reservations"], w["dispatcher"]), "V08.3: second loop still rejected after owner reset (claim held while alive)")
	w["dispatcher"].assignment_arrived.disconnect(diag)
	l1 = null; l3 = null
	# Fresh-loop-after-GC + no-leak proof -> tests/m20_v08_lifecycle_smoke.gd.
	print("  V08.3: owner-GC claim release + no-leak proven in tests/m20_v08_lifecycle_smoke.gd")
	_m20_teardown(w)

# --- §4 agent_parent lifecycle -----------------------------------------------
func _v08_parts_reset_access(board) -> Dictionary:
	var reservations = ReservationState.new(); reservations.bind(board)
	var candidates = ColorCandidateIndex.create(); candidates.bind(board)
	var selector = TargetSelector.create(); selector.bind(board, candidates, reservations)
	var routing = ProductionRoutingSystem.new()
	var ra = ProductionAccessQuery.new(board)
	var sa = M20ResetSelectAccess.new(routing, ra, board)
	var d = ScrubbotDispatcher.new(); root.add_child(d)
	return {"d": d, "selector": selector, "reservations": reservations, "candidates": candidates,
		"routing": routing, "ra": ra, "sa": sa}

func _v08_agent_parent() -> void:
	print("---- V08 §4: agent_parent lifecycle ----")
	# omitted -> self.
	var b1 = _m19_open_board_active(22, 22, [Vector2(11, 11)])
	var c1: int = b1.get_color_id(b1.get_cell_index(11, 11))
	var p1 = _m19_parts(b1)
	_check(p1["d"].bind(b1, p1["selector"], p1["reservations"], p1["routing"], p1["ra"], p1["sa"]), "V08.4: omitted parent binds")
	_check(p1["d"].dispatch(c1, Vector2(-2.0, 11.5), 6.0).agent.get_parent() == p1["d"], "V08.4: omitted -> agent under self")
	_m19_parts_teardown(p1)
	# explicit null -> self.
	var b2 = _m19_open_board_active(22, 22, [Vector2(11, 11)])
	var c2: int = b2.get_color_id(b2.get_cell_index(11, 11))
	var p2 = _m19_parts(b2)
	_check(p2["d"].bind(b2, p2["selector"], p2["reservations"], p2["routing"], p2["ra"], p2["sa"], null), "V08.4: explicit null binds")
	_check(p2["d"].dispatch(c2, Vector2(-2.0, 11.5), 6.0).agent.get_parent() == p2["d"], "V08.4: explicit null -> self")
	_m19_parts_teardown(p2)
	# healthy explicit Node -> attaches there.
	var b3 = _m19_open_board_active(22, 22, [Vector2(11, 11)])
	var c3: int = b3.get_color_id(b3.get_cell_index(11, 11))
	var p3 = _m19_parts(b3)
	var parent = Node.new(); root.add_child(parent)
	_check(p3["d"].bind(b3, p3["selector"], p3["reservations"], p3["routing"], p3["ra"], p3["sa"], parent), "V08.4: healthy explicit Node binds")
	_check(p3["d"].dispatch(c3, Vector2(-2.0, 11.5), 6.0).agent.get_parent() == parent, "V08.4: agent attaches under explicit parent")
	_m19_parts_teardown(p3); root.remove_child(parent); parent.free()
	# scalar / RefCounted / queued rejected.
	for kase in [["scalar", 123], ["RefCounted", RefCounted.new()]]:
		var bk = _m19_open_board_active(22, 22, [Vector2(11, 11)])
		var pk = _m19_parts(bk)
		_check(not pk["d"].bind(bk, pk["selector"], pk["reservations"], pk["routing"], pk["ra"], pk["sa"], kase[1]), "V08.4: %s explicit parent rejected" % kase[0])
		_m19_parts_teardown(pk)
	var bq = _m19_open_board_active(22, 22, [Vector2(11, 11)])
	var pq = _m19_parts(bq)
	var qn = Node.new(); root.add_child(qn); qn.queue_free()
	_check(not pq["d"].bind(bq, pq["selector"], pq["reservations"], pq["routing"], pq["ra"], pq["sa"], qn), "V08.4: queued explicit parent rejected")
	root.remove_child(qn); qn.free(); _m19_parts_teardown(pq)
	# explicit parent QUEUED from a bind-time coherence callback -> bind false.
	var bc = _m19_open_board_active(22, 22, [Vector2(11, 11)])
	var pc = _v08_parts_reset_access(bc)
	var cparent = Node.new(); root.add_child(cparent)
	pc["sa"].coherence_hook = func(): cparent.queue_free()
	_check(not pc["d"].bind(bc, pc["selector"], pc["reservations"], pc["routing"], pc["ra"], pc["sa"], cparent), "V08.4: parent queued from bind-time coherence callback -> bind false")
	_check(not pc["d"].is_bound(), "V08.4: dispatcher remains unbound")
	root.remove_child(cparent); cparent.free(); _m19_parts_teardown(pc)
	# truly-freed parent before bind + parent destroyed before dispatch -> smoke.
	print("  V08.4: truly-freed explicit parent + destroy-before-dispatch proven in tests/m20_v08_lifecycle_smoke.gd")

# --- §5 renderer lifecycle ---------------------------------------------------
func _v08_renderer() -> void:
	print("---- V08 §5: renderer lifecycle ----")
	var board = _m19_open_board_active(22, 22, [Vector2(11, 11)])
	var color: int = board.get_color_id(board.get_cell_index(11, 11))
	# headless (omitted + null) success.
	var w0 = _m20_full(board)
	var lo = CompleteClearingLoop.new()
	_check(lo.bind(board, _m20_slots([color, color, color, color, color]), w0["candidates"], w0["reservations"], w0["dispatcher"]), "V08.5: omitted renderer headless bind")
	lo = null; _m20_teardown(w0)
	var wn = _m20_full(_m19_open_board_active(22, 22, [Vector2(11, 11)]))
	var ln = CompleteClearingLoop.new()
	_check(ln.bind(wn["board"], _m20_slots([color, color, color, color, color]), wn["candidates"], wn["reservations"], wn["dispatcher"], null), "V08.5: explicit-null headless bind")
	ln = null; _m20_teardown(wn)
	# invalid variants rejected.
	var wv = _m20_full(_m19_open_board_active(22, 22, [Vector2(11, 11)]))
	var variants: Array = [7, 2.5, "s", Vector2(1, 1), [1], {"k": 0}, RefCounted.new(), wv["dispatcher"]]
	for v in variants:
		var lv = CompleteClearingLoop.new()
		_check(not lv.bind(wv["board"], _m20_slots([color, color, color, color, color]), wv["candidates"], wv["reservations"], wv["dispatcher"], v), "V08.5: invalid renderer variant %s rejected" % typeof(v))
	_check_eq(_m20_conn_count(wv["dispatcher"]), 0, "V08.5: no invalid variant connected")
	_m20_teardown(wv)
	# queued renderer before bind rejected.
	var wq = _m20_full(_m19_open_board_active(22, 22, [Vector2(11, 11)]), null, null, true)
	wq["renderer"].queue_free()
	_check(not CompleteClearingLoop.new().bind(wq["board"], _m20_slots([color, color, color, color, color]), wq["candidates"], wq["reservations"], wq["dispatcher"], wq["renderer"]), "V08.5: queued renderer before bind rejected")
	root.remove_child(wq["renderer"]); wq["renderer"].free(); _m20_teardown(wq)
	# failed-bind metadata recovery on same loop.
	var wr = _m20_full(_m19_open_board_active(22, 22, [Vector2(11, 11)]))
	var loop = CompleteClearingLoop.new()
	_check(not loop.bind(wr["board"], _m20_slots([color, color, color, color, color]), wr["candidates"], wr["reservations"], wr["dispatcher"], 42), "V08.5: invalid renderer fails")
	_check(loop.bind(wr["board"], _m20_slots([color, color, color, color, color]), wr["candidates"], wr["reservations"], wr["dispatcher"]), "V08.5: same loop then clean headless bind")
	_check(loop.is_coherent(), "V08.5: no stale renderer metadata")
	loop = null; _m20_teardown(wr)
	# second-bind preservation via foreign-board drift AND via queued original.
	var ws = _m20_full(_m19_open_board_active(22, 22, [Vector2(11, 11)]), null, null, true)
	var ls = _m20_loop(ws, _m20_slots([color, color, color, color, color]))
	_check(not ls.bind(ws["board"], _m20_slots([color, color, color, color, color]), ws["candidates"], ws["reservations"], ws["dispatcher"], null), "V08.5: second bind refused")
	var other = _m19_open_board_active(22, 22, [Vector2(3, 3)])
	ws["renderer"].configure(other, PackedStringArray(BoardDebugFixturesM20.PALETTE), Vector2(100, 100))
	_check(not ls.is_coherent(), "V08.5: original configured renderer still required (foreign-board drift)")
	ls = null; root.remove_child(ws["renderer"]); ws["renderer"].free(); ws["renderer"] = null; _m20_teardown(ws)
	var wsq = _m20_full(_m19_open_board_active(22, 22, [Vector2(11, 11)]), null, null, true)
	var lsq = _m20_loop(wsq, _m20_slots([color, color, color, color, color]))
	wsq["renderer"].queue_free()
	_check(not lsq.is_coherent(), "V08.5: preservation via queued/dead original renderer")
	lsq = null; if is_instance_valid(wsq["renderer"]): root.remove_child(wsq["renderer"]); wsq["renderer"].free()
	wsq["renderer"] = null; _m20_teardown(wsq)
	# assignment then renderer queued before arrival -> no clear; candidate present; held.
	var wa = _m20_full(_m19_open_board_active(22, 22, [Vector2(11, 11)]), null, null, true)
	var ta: int = wa["board"].get_cell_index(11, 11)
	var ca: int = wa["board"].get_color_id(ta)
	var la = _m20_loop(wa, _m20_slots([ca, ca, ca, ca, ca]))
	var ra2 = la.activate_slot(0, Vector2(-2.0, 11.5), 6.0)
	wa["renderer"].queue_free()
	_m19_drive_to_arrival(ra2.agent)
	_check_eq(wa["board"].get_cell_state(ta), BoardState.CellState.ACTIVE, "V08.5: renderer queued before arrival -> no clear")
	_check(wa["candidates"].get_candidates(ca, null).has(ta), "V08.5: candidate still present")
	_check_eq(wa["reservations"].get_owner(ta), ra2.owner_id, "V08.5: exact reservation held")
	_check(wa["dispatcher"].has_owner(ra2.owner_id), "V08.5: dispatcher assignment held")
	_check_eq(la.get_cleared_count(), 0, "V08.5: cleared_count unchanged")
	la.reset()
	_check_eq(wa["dispatcher"].get_active_count(), 0, "V08.5: reset cleans the held pair/agent")
	if is_instance_valid(wa["renderer"]): root.remove_child(wa["renderer"]); wa["renderer"].free()
	wa["renderer"] = null; la = null; _m20_teardown(wa)
	# healthy renderer clear -> target alpha 0, unrelated pixel unchanged.
	var wh = _m20_full(_m19_open_board_active(22, 22, [Vector2(11, 11), Vector2(15, 13)]), null, null, true)
	var th: int = wh["board"].get_cell_index(11, 11)
	var ch: int = wh["board"].get_color_id(th)
	var uh: int = wh["board"].get_cell_index(15, 13)
	var lh = _m20_loop(wh, _m20_slots([ch, ch, ch, ch, ch]))
	var upos: Vector2i = wh["board"].get_cell_position(uh)
	var ubefore: Color = wh["renderer"].get_pixel_color(upos.x, upos.y)
	_check(_m20_activate_and_arrive(lh, 0, Vector2(-2.0, 11.5)).success, "V08.5: healthy renderer clear")
	var tpos: Vector2i = wh["board"].get_cell_position(th)
	_check(_colors_close(wh["renderer"].get_pixel_color(tpos.x, tpos.y), Color(0, 0, 0, 0), 0.02), "V08.5: target alpha 0")
	_check_eq(wh["renderer"].get_pixel_color(upos.x, upos.y), ubefore, "V08.5: unrelated pixel unchanged")
	lh = null; _m20_teardown(wh)

# --- §6 arrival-preflight desync matrix --------------------------------------
## Build a real ARRIVED-but-unconsumed assignment: disconnect the loop, drive to
## arrival. Returns {w, loop, owner, target, color, agent}.
func _v08_arrived(board, extra_active: Array = []) -> Dictionary:
	var w = _m20_full(board)
	var target: int = board.get_cell_index(11, 11)
	var color: int = board.get_color_id(target)
	var loop = _m20_loop(w, _m20_slots([color, color, color, color, color]))
	var r = loop.activate_slot(0, Vector2(-2.0, 11.5), 6.0)
	w["dispatcher"].assignment_arrived.disconnect(loop._arrival_cb)
	_m19_drive_to_arrival(r.agent)
	return {"w": w, "loop": loop, "owner": r.owner_id, "target": target, "color": color, "agent": r.agent}

func _v08_desync() -> void:
	print("---- V08 §6: arrival-preflight desync matrix ----")
	# 1-5 identity variants + unknown owner.
	var a = _v08_arrived(_m19_open_board_active(22, 22, [Vector2(11, 11)]))
	var w = a["w"]; var loop = a["loop"]; var o = a["owner"]; var t = a["target"]; var c = a["color"]; var ag = a["agent"]
	for v in [[o + 1, t, c, ag, "wrong owner"], [o, t + 1, c, ag, "wrong target"], [o, t, c + 1, ag, "wrong color"], [o, t, c, null, "wrong agent"], [4242, t, c, ag, "unknown owner"]]:
		var before = _snapshot_cell_states(w["board"])
		loop._on_assignment_arrived(v[0], v[1], v[2], v[3])
		_check_eq(loop.get_last_outcome(), CompleteClearingLoop.Outcome.PREFLIGHT_REJECTED, "V08.6: %s -> rejected" % v[4])
		_check(_cell_states_equal(w["board"], before), "V08.6: %s -> no board mutation" % v[4])
	_check(w["dispatcher"].has_owner(o), "V08.6: identity variants did not finalize the assignment")
	_check_eq(loop.get_cleared_count(), 0, "V08.6: identity variants cleared nothing")
	loop = null; _m20_teardown(w)

	# 7. same board: release (T,O), reserve SAME T for O2; inject O arrival -> reject;
	#    later reset preserves T->O2.
	var a7 = _v08_arrived(_m19_open_board_active(22, 22, [Vector2(11, 11)]))
	a7["w"]["reservations"].release(a7["target"], a7["owner"])
	var o2 := 5551
	a7["w"]["reservations"].reserve(a7["target"], o2)
	a7["loop"]._on_assignment_arrived(a7["owner"], a7["target"], a7["color"], a7["agent"])
	_check_eq(a7["loop"].get_last_outcome(), CompleteClearingLoop.Outcome.PREFLIGHT_REJECTED, "V08.6.7: same-board T->O2 -> original arrival rejected")
	a7["w"]["dispatcher"].assignment_arrived.connect(a7["loop"]._arrival_cb)
	a7["loop"].reset()
	_check_eq(a7["w"]["reservations"].get_owner(a7["target"]), o2, "V08.6.7: reset preserves T->O2")
	a7["loop"] = null; _m20_teardown(a7["w"])

	# 8. reservations rebind(foreign) + foreign reservation; inject -> reject; reset preserves foreign.
	var a8 = _v08_arrived(_m19_open_board_active(22, 22, [Vector2(11, 11)]))
	var fb = _m19_open_board_active(22, 22, [Vector2(4, 4)])
	a8["w"]["reservations"].rebind(fb)
	var fv: int = fb.get_cell_index(4, 4)
	a8["w"]["reservations"].reserve(fv, a8["owner"])
	a8["loop"]._on_assignment_arrived(a8["owner"], a8["target"], a8["color"], a8["agent"])
	_check_eq(a8["loop"].get_last_outcome(), CompleteClearingLoop.Outcome.PREFLIGHT_REJECTED, "V08.6.8: foreign-board reservation -> rejected")
	a8["w"]["dispatcher"].assignment_arrived.connect(a8["loop"]._arrival_cb)
	a8["loop"].reset()
	_check_eq(a8["w"]["reservations"].get_owner(fv), a8["owner"], "V08.6.8: reset preserves foreign reservation")
	a8["loop"] = null; _m20_teardown(a8["w"])

	# 9. candidates rebind(foreign) -> reject; board/reservation/dispatcher untouched until reset.
	var a9 = _v08_arrived(_m19_open_board_active(22, 22, [Vector2(11, 11)]))
	a9["w"]["candidates"].rebind(_m19_open_board_active(22, 22, [Vector2(2, 2)]))
	a9["loop"]._on_assignment_arrived(a9["owner"], a9["target"], a9["color"], a9["agent"])
	_check_eq(a9["loop"].get_last_outcome(), CompleteClearingLoop.Outcome.PREFLIGHT_REJECTED, "V08.6.9: candidate foreign-board -> rejected")
	_check_eq(a9["w"]["board"].get_cell_state(a9["target"]), BoardState.CellState.ACTIVE, "V08.6.9: target untouched")
	_check(a9["w"]["dispatcher"].has_owner(a9["owner"]), "V08.6.9: dispatcher current attempt untouched")
	a9["loop"] = null; _m20_teardown(a9["w"])

	# 10. candidates rebind(null) -> reject cleanly.
	var a10 = _v08_arrived(_m19_open_board_active(22, 22, [Vector2(11, 11)]))
	a10["w"]["candidates"].rebind(null)
	a10["loop"]._on_assignment_arrived(a10["owner"], a10["target"], a10["color"], a10["agent"])
	_check_eq(a10["loop"].get_last_outcome(), CompleteClearingLoop.Outcome.PREFLIGHT_REJECTED, "V08.6.10: candidate rebind(null) -> rejected cleanly")
	_check_eq(a10["w"]["board"].get_cell_state(a10["target"]), BoardState.CellState.ACTIVE, "V08.6.10: target ACTIVE")
	a10["loop"] = null; _m20_teardown(a10["w"])

	# 11. target externally CLEARED before arrival.
	var a11 = _v08_arrived(_m19_open_board_active(22, 22, [Vector2(11, 11)]))
	a11["w"]["board"].set_cell_state(a11["target"], BoardState.CellState.CLEARED)
	a11["loop"]._on_assignment_arrived(a11["owner"], a11["target"], a11["color"], a11["agent"])
	_check_eq(a11["loop"].get_last_outcome(), CompleteClearingLoop.Outcome.PREFLIGHT_REJECTED, "V08.6.11: externally CLEARED -> rejected")
	_check_eq(a11["loop"].get_cleared_count(), 0, "V08.6.11: cleared_count unchanged")
	a11["loop"] = null; _m20_teardown(a11["w"])

	# 14. stale replay after reset. 15. duplicate current completion (idempotent).
	var a14 = _v08_arrived(_m19_open_board_active(22, 22, [Vector2(11, 11)]))
	a14["w"]["dispatcher"].assignment_arrived.connect(a14["loop"]._arrival_cb)
	a14["loop"].reset()
	a14["loop"]._on_assignment_arrived(a14["owner"], a14["target"], a14["color"], a14["agent"])
	_check_eq(a14["loop"].get_cleared_count(), 0, "V08.6.14: stale replay after reset cannot clear")
	a14["loop"] = null; _m20_teardown(a14["w"])

# --- §7 activation public-boundary matrix (per-case snapshots) ----------------
func _v08_snap(w) -> Dictionary:
	var reserved := {}
	for idx in w["reservations"].get_reserved_indices():
		reserved[int(idx)] = w["reservations"].get_owner(int(idx))
	return {"cells": _snapshot_cell_states(w["board"]), "active": w["dispatcher"].get_active_count(),
		"next_owner": w["dispatcher"].peek_next_owner_id(), "reserved": reserved}

func _v08_snap_unchanged(w, s: Dictionary, name: String) -> void:
	_check(_cell_states_equal(w["board"], s["cells"]), "V08.7: %s -> BoardState unchanged" % name)
	_check_eq(w["dispatcher"].get_active_count(), s["active"], "V08.7: %s -> active unchanged" % name)
	_check_eq(w["dispatcher"].peek_next_owner_id(), s["next_owner"], "V08.7: %s -> owner id unchanged" % name)

func _v08_activation() -> void:
	print("---- V08 §7: activation public-boundary matrix (per-case snapshots) ----")
	var board = _m19_open_board_active(22, 22, [Vector2(11, 11)])
	var color: int = board.get_color_id(board.get_cell_index(11, 11))
	var w = _m20_full(board)
	var loop = _m20_loop(w, _m20_slots([color, color, color, color, color]))
	var cases: Array = [
		["slot -1", func(): return loop.activate_slot(-1, Vector2(-2.0, 11.5))],
		["slot 5", func(): return loop.activate_slot(5, Vector2(-2.0, 11.5))],
		["non-int slot", func(): return loop.activate_slot(1.5, Vector2(-2.0, 11.5))],
		["origin NaN x", func(): return loop.activate_slot(0, Vector2(NAN, 11.5))],
		["origin NaN y", func(): return loop.activate_slot(0, Vector2(11.5, NAN))],
		["origin +INF", func(): return loop.activate_slot(0, Vector2(INF, 11.5))],
		["origin -INF", func(): return loop.activate_slot(0, Vector2(-INF, 11.5))],
		["speed NaN", func(): return loop.activate_slot(0, Vector2(-2.0, 11.5), NAN)],
		["speed +INF", func(): return loop.activate_slot(0, Vector2(-2.0, 11.5), INF)],
		["speed -INF", func(): return loop.activate_slot(0, Vector2(-2.0, 11.5), -INF)],
		["speed 0", func(): return loop.activate_slot(0, Vector2(-2.0, 11.5), 0.0)],
		["speed negative", func(): return loop.activate_slot(0, Vector2(-2.0, 11.5), -3.0)],
	]
	for kase in cases:
		var s = _v08_snap(w)
		var res = kase[1].call()
		_check(not res.success, "V08.7: %s rejected" % kase[0])
		_v08_snap_unchanged(w, s, kase[0])
	# unavailable slot (fresh, with per-case snapshot).
	var sysu = _m20_slots([color, color, color, color, color]); sysu.set_slot_available(0, false)
	var wu = _m20_full(_m19_open_board_active(22, 22, [Vector2(11, 11)]))
	var lu = _m20_loop(wu, sysu)
	var su = _v08_snap(wu)
	_check(not lu.activate_slot(0, Vector2(-2.0, 11.5)).success, "V08.7: unavailable slot rejected")
	_v08_snap_unchanged(wu, su, "unavailable slot")
	# slot fields unmutated by rejections.
	_check_eq(sysu.get_slot_palette_id(0), color, "V08.7: rejected activation did not mutate slot palette")
	_check(not sysu.is_slot_available(0), "V08.7: rejected activation did not mutate slot availability")
	lu = null; _m20_teardown(wu)
	# nested activation + drain-active + control valid.
	var seam = M20CandidateSeam.new(); seam.bind(board)
	var wn = _m20_full(_m19_open_board_active(22, 22, [Vector2(11, 11), Vector2(13, 11)]), seam)
	var ln = _m20_harness_bind(wn, _m20_slots([wn["board"].get_color_id(wn["board"].get_cell_index(11, 11)), 0, 0, 0, 0]))
	var reason := [&""]
	seam.coherence_hook = func(): reason[0] = ln.activate_slot(1, Vector2(26.0, 11.5)).failure_reason
	var outer = ln.activate_slot(0, Vector2(-2.0, 11.5), 6.0)
	_check_eq(reason[0], DispatchResult.FailureReason.REENTRANT, "V08.7: nested activation -> REENTRANT")
	seam.coherence_hook = Callable(); ln = null; _m20_teardown(wn)
	# control valid finite activation.
	_check(loop.activate_slot(0, Vector2(-2.0, 11.5), 6.0).success, "V08.7: control valid activation succeeds")
	loop = null; _m20_teardown(w)
	# reset inside M19 dispatch -> RESETTING (owner consumed, monotonic).
	var wr = _m20_full_reset_access(_m19_open_board_active(22, 22, [Vector2(11, 11), Vector2(13, 11)]))
	var lr = _m20_loop(wr, _m20_slots([wr["board"].get_color_id(wr["board"].get_cell_index(11, 11)), 0, 0, 0, 0]))
	var ob: int = wr["dispatcher"].peek_next_owner_id()
	wr["select_access"].hook = func(): lr.reset()
	var rr = lr.activate_slot(0, Vector2(-2.0, 11.5), 6.0)
	_check(not rr.success and rr.failure_reason == DispatchResult.FailureReason.RESETTING, "V08.7: reset inside M19 dispatch -> RESETTING (no stale success)")
	_check(wr["dispatcher"].peek_next_owner_id() > ob, "V08.7: consumed owner id is monotonic (not reused)")
	_check_eq(wr["dispatcher"].get_active_count(), 0, "V08.7: reset-inside-dispatch left no active assignment")
	wr["select_access"].hook = Callable(); lr = null; _m20_teardown(wr)

# --- §8 SB-M20 final ledger (compact fresh) ----------------------------------
func _v08_ledger() -> void:
	print("---- V08 §8: SB-M20-001..014 final ledger ----")
	# 001 healthy synchronized tuple.
	var b1 = _m19_open_board_active(24, 24, [Vector2(12, 12)])
	var t1: int = b1.get_cell_index(12, 12); var c1: int = b1.get_color_id(t1)
	var w1 = _m20_full(b1, null, null, true)
	var l1 = _m20_loop(w1, _m20_slots([c1, c1, c1, c1, c1]))
	var r1 = _m20_activate_and_arrive(l1, 0, Vector2(-2.0, 12.5))
	var p1: Vector2i = b1.get_cell_position(t1)
	_check(r1.success and b1.get_cell_state(t1) == BoardState.CellState.CLEARED and not w1["candidates"].get_candidates(c1, null).has(t1) and w1["routing_access"].classify_cell(p1.x, p1.y, -1) == ProductionAccessQuery.CellClass.OPEN and w1["reservations"].get_owner(t1) == -1 and not w1["dispatcher"].has_owner(r1.owner_id) and r1.agent.is_queued_for_deletion() and _colors_close(w1["renderer"].get_pixel_color(p1.x, p1.y), Color(0, 0, 0, 0), 0.02), "SB-M20-001: full synchronized tuple")
	l1 = null; _m20_teardown(w1)
	# 002 no-target + enclosed.
	var b2 = _m19_open_board_active(22, 22, [Vector2(10, 2)])
	var w2 = _m20_full(b2)
	var miss: int = (b2.get_color_id(b2.get_cell_index(10, 2)) + 1) % 5
	var l2 = _m20_loop(w2, _m20_slots([miss, miss, miss, miss, miss]))
	_check_eq(l2.activate_slot(0, Vector2(-2.0, 2.5)).failure_reason, DispatchResult.FailureReason.NO_REACHABLE_TARGET, "SB-M20-002: no target -> no bot")
	l2 = null; _m20_teardown(w2)
	var w2e = _m20_full(_m19_enclosed_sole_candidate_board())
	var l2e = _m20_loop(w2e, _m20_slots([1, 1, 1, 1, 1]))
	_check_eq(l2e.activate_slot(0, Vector2(-2.0, 2.5)).failure_reason, DispatchResult.FailureReason.NO_REACHABLE_TARGET, "SB-M20-002: enclosed -> no bot")
	l2e = null; _m20_teardown(w2e)
	print("  SB-M20-003: frame no-return/deferred disappearance -> tests/m20_queue_free_smoke.gd + tests/m20_v08_lifecycle_smoke.gd")
	# 004 1x1.
	var b4 = _m19_open_board_active(1, 1, [Vector2(0, 0)])
	var w4 = _m20_full(b4)
	var l4 = _m20_loop(w4, _m20_slots([b4.get_color_id(0), 0, 0, 0, 0]))
	_check(_m20_activate_and_arrive(l4, 0, Vector2(-2.0, 0.5)).success and l4.get_cleared_count() == 1, "SB-M20-004: 1x1 clears")
	_check_eq(l4.activate_slot(0, Vector2(-2.0, 0.5)).failure_reason, DispatchResult.FailureReason.NO_REACHABLE_TARGET, "SB-M20-004: exhausted")
	l4 = null; _m20_teardown(w4)
	# 005 one-color exhaustion.
	var cells5: Array = []
	for x in range(7): cells5.append(Vector2(x + 2, 6))
	var b5 = _m19_open_board_active(22, 22, cells5)
	var c5: int = b5.get_color_id(b5.get_cell_index(2, 6))
	var w5 = _m20_full(b5); var l5 = _m20_loop(w5, _m20_slots([c5, c5, c5, c5, c5]))
	var n5 := 0
	for _i in range(20):
		if not _m20_activate_and_arrive(l5, 0, Vector2(-2.0, 6.5)).success: break
		n5 += 1
	_check(n5 == 7 and b5.count_cells_by_state(BoardState.CellState.ACTIVE) == 0, "SB-M20-005: one-color exhausted (7)")
	l5 = null; _m20_teardown(w5)
	# 006 multi-color.
	var b6 = _m19_open_board_active(22, 22, [Vector2(10, 0), Vector2(10, 1), Vector2(10, 2)])
	var w6 = _m20_full(b6)
	var l6 = _m20_loop(w6, _m20_slots([b6.get_color_id(b6.get_cell_index(10, 0)), b6.get_color_id(b6.get_cell_index(10, 1)), b6.get_color_id(b6.get_cell_index(10, 2)), 0, 0]))
	_check(_m20_activate_and_arrive(l6, 0, Vector2(-2.0, 0.5)).target_index == b6.get_cell_index(10, 0) and _m20_activate_and_arrive(l6, 1, Vector2(-2.0, 1.5)).target_index == b6.get_cell_index(10, 1) and _m20_activate_and_arrive(l6, 2, Vector2(-2.0, 2.5)).target_index == b6.get_cell_index(10, 2), "SB-M20-006: multi-color no cross corruption")
	l6 = null; _m20_teardown(w6)
	# 007 five in-flight.
	var b7 = _m19_open_board_active(24, 24, [Vector2(12, 0), Vector2(12, 1), Vector2(12, 2), Vector2(12, 3), Vector2(12, 4)])
	var pal7: Array = []
	for y in range(5): pal7.append(b7.get_color_id(b7.get_cell_index(12, y)))
	var w7 = _m20_full(b7); var l7 = _m20_loop(w7, _m20_slots(pal7))
	var org7: Array = [Vector2(-2.0, 0.5), Vector2(26.0, 1.5), Vector2(-2.0, 2.5), Vector2(26.0, 3.5), Vector2(-2.0, 4.5)]
	var res7: Array = []; var ow := {}; var tg := {}
	for i in range(5):
		var r = l7.activate_slot(i, org7[i], 6.0)
		if r.success: res7.append(r); ow[r.owner_id] = true; tg[r.target_index] = true
	var pairs7 := true
	for r in res7:
		if w7["reservations"].get_owner(r.target_index) != r.owner_id: pairs7 = false
	_check(res7.size() == 5 and ow.size() == 5 and tg.size() == 5 and pairs7, "SB-M20-007: five unique owners/targets/exact pairs")
	_m19_drive_to_arrival(res7[0].agent)
	var four := true
	for i in range(1, 5):
		if not w7["dispatcher"].has_owner(res7[i].owner_id): four = false
	_check(four, "SB-M20-007: first resolves, four preserved")
	for i in range(1, 5): _m19_drive_to_arrival(res7[i].agent)
	_check_eq(l7.get_cleared_count(), 5, "SB-M20-007: all five resolve")
	l7 = null; _m20_teardown(w7)
	# 008-011 dims.
	for d in [[24, 24, "008 Easy"], [34, 34, "009 Medium"], [44, 44, "010 Hard"], [54, 54, "011 VeryHard"]]:
		var bd = _m19_open_board_active(d[0], d[1], [Vector2(13, 10)])
		var wd = _m20_full(bd); var ld = _m20_loop(wd, _m20_slots([bd.get_color_id(bd.get_cell_index(13, 10)), 0, 0, 0, 0]))
		_check(_m20_activate_and_arrive(ld, 0, Vector2(-2.0, 10.5)).success and ld.get_cleared_count() == 1, "SB-M20-%s clears" % d[2])
		ld = null; _m20_teardown(wd)
	# 012 59x59 single-cell.
	var big = _m19_open_board_active(59, 59, [Vector2(29, 29)])
	var wbig = _m20_full(big); var lbig = _m20_loop(wbig, _m20_slots([big.get_color_id(big.get_cell_index(29, 29)), 0, 0, 0, 0]))
	var bb: int = big.count_cells_by_state(BoardState.CellState.CLEARED)
	_check(_m20_activate_and_arrive(lbig, 0, Vector2(-2.0, 29.5)).success and big.count_cells_by_state(BoardState.CellState.CLEARED) == bb + 1, "SB-M20-012: 59x59 single-cell path")
	lbig = null; _m20_teardown(wbig)
	# 013 rectangular.
	var rect = _m19_open_board_active(53, 59, [Vector2(20, 30)])
	var wrect = _m20_full(rect); var lrect = _m20_loop(wrect, _m20_slots([rect.get_color_id(rect.get_cell_index(20, 30)), 0, 0, 0, 0]))
	_check(_m20_activate_and_arrive(lrect, 0, Vector2(-2.0, 30.5)).success, "SB-M20-013: rectangular clears")
	lrect = null; _m20_teardown(wrect)
	# 014 AL-028 second-B + rapid + reset-with-multiple + slot-fields-immutable.
	var bA = _m19_open_board_active(8, 8, [Vector2(1, 2), Vector2(2, 2), Vector2(3, 2), Vector2(2, 1), Vector2(2, 3)])
	var door: int = bA.get_cell_index(1, 2); var inner: int = bA.get_cell_index(2, 2)
	var wA = _m20_full(bA); var slotsA = _m20_slots([bA.get_color_id(door), bA.get_color_id(door), bA.get_color_id(door), bA.get_color_id(door), bA.get_color_id(door)])
	var lA = _m20_loop(wA, slotsA)
	_check(_m20_activate_and_arrive(lA, 0, Vector2(-2.0, 2.5)).target_index == door, "SB-M20-014: AL-028 A cleared")
	var rB = lA.activate_slot(1, Vector2(-2.0, 2.5), 6.0)
	_check(rB.success and rB.target_index == inner, "SB-M20-014: second real activation selects B")
	_m19_drive_to_arrival(rB.agent)
	_check_eq(bA.get_cell_state(inner), BoardState.CellState.CLEARED, "SB-M20-014: B cleared")
	# slot fields not mutated by M20.
	_check(slotsA.is_slot_available(0) and slotsA.get_slot_palette_id(0) == bA.get_color_id(door) and not slotsA.is_slot_active(0), "SB-M20-014: M20 did not mutate slot palette/availability/activity")
	lA = null; _m20_teardown(wA)
	# reset with multiple in-flight.
	var bm = _m19_open_board_active(22, 22, [Vector2(11, 0), Vector2(11, 1), Vector2(11, 2)])
	var wm = _m20_full(bm)
	var lm = _m20_loop(wm, _m20_slots([bm.get_color_id(bm.get_cell_index(11, 0)), bm.get_color_id(bm.get_cell_index(11, 1)), bm.get_color_id(bm.get_cell_index(11, 2)), 0, 0]))
	for i in range(3): lm.activate_slot(i, Vector2(-2.0, float(i) + 0.5), 6.0)
	_check_eq(wm["dispatcher"].get_active_count(), 3, "SB-M20-014: three in-flight before reset")
	lm.reset()
	_check(wm["dispatcher"].get_active_count() == 0 and wm["reservations"].get_reservation_count() == 0, "SB-M20-014: reset clears multiple in-flight")
	lm = null; _m20_teardown(wm)
	# rapid >=25.
	var rc: Array = []
	for x in range(28): rc.append(Vector2(x + 1, 6))
	var brap = _m19_open_board_active(30, 12, rc)
	var wrap = _m20_full(brap); var lrap = _m20_loop(wrap, _m20_slots([brap.get_color_id(brap.get_cell_index(1, 6)), 0, 0, 0, 0]))
	var cyc := 0
	for _i in range(40):
		if not _m20_activate_and_arrive(lrap, 0, Vector2(-2.0, 6.5)).success: break
		cyc += 1
	_check(cyc >= 25 and lrap.get_cleared_count() == 28, "SB-M20-014: rapid >=25 cycles (%d)" % cyc)
	lrap = null; _m20_teardown(wrap)

# --- §9 rollback/reset/contention high-risk ----------------------------------
func _v08_rollback() -> void:
	print("---- V08 §9: rollback/reset/contention ----")
	# candidate mutate-before-false -> verified rollback.
	var b1 = _m19_open_board_active(22, 22, [Vector2(11, 11)])
	var t1: int = b1.get_cell_index(11, 11); var c1: int = b1.get_color_id(t1)
	var cs1 = M20CandidateSeam.new(); cs1.bind(b1)
	var w1 = _m20_full(b1, cs1)
	var l1 = _m20_harness_bind(w1, _m20_slots([c1, c1, c1, c1, c1]))
	var r1 = l1.activate_slot(0, Vector2(-2.0, 11.5), 6.0)
	cs1.mode = "mutate_false"
	_m19_drive_to_arrival(r1.agent)
	_check_eq(l1.get_last_outcome(), CompleteClearingLoop.Outcome.CANDIDATE_ROLLBACK, "V08.9: candidate mutate-before-false -> verified rollback")
	_check_eq(b1.get_cell_state(t1), BoardState.CellState.ACTIVE, "V08.9: candidate rollback -> ACTIVE")
	cs1.mode = "normal"; l1 = null; _m20_teardown(w1)
	# reservation mutate-before-false.
	var b2 = _m19_open_board_active(22, 22, [Vector2(11, 11)])
	var t2: int = b2.get_cell_index(11, 11); var c2: int = b2.get_color_id(t2)
	var rs2 = M20ReservationSeam.new(); rs2.bind(b2)
	var w2 = _m20_full(b2, null, rs2)
	var l2 = _m20_harness_bind(w2, _m20_slots([c2, c2, c2, c2, c2]))
	var r2 = l2.activate_slot(0, Vector2(-2.0, 11.5), 6.0)
	rs2.mode = "mutate_false"
	_m19_drive_to_arrival(r2.agent)
	_check_eq(l2.get_last_outcome(), CompleteClearingLoop.Outcome.RESERVATION_ROLLBACK, "V08.9: reservation mutate-before-false -> verified rollback")
	rs2.mode = "normal"; l2 = null; _m20_teardown(w2)
	# candidate true-without-postcondition.
	var b3 = _m19_open_board_active(22, 22, [Vector2(11, 11)])
	var t3: int = b3.get_cell_index(11, 11); var c3: int = b3.get_color_id(t3)
	var cs3 = M20CandidateSeam.new(); cs3.bind(b3)
	var w3 = _m20_full(b3, cs3)
	var l3 = _m20_harness_bind(w3, _m20_slots([c3, c3, c3, c3, c3]))
	var r3 = l3.activate_slot(0, Vector2(-2.0, 11.5), 6.0)
	cs3.mode = "true_noop"
	_m19_drive_to_arrival(r3.agent)
	_check_eq(l3.get_last_outcome(), CompleteClearingLoop.Outcome.CANDIDATE_ROLLBACK, "V08.9: candidate true-without-postcondition -> rollback")
	cs3.mode = "normal"; l3 = null; _m20_teardown(w3)
	# reservation identity-swap -> ROLLBACK_FAILED (owner-map proof).
	var b4 = _m19_open_board_active(22, 22, [Vector2(9, 11), Vector2(11, 11), Vector2(13, 11)])
	var t4T: int = b4.get_cell_index(9, 11); var t4U: int = b4.get_cell_index(11, 11); var t4V: int = b4.get_cell_index(13, 11)
	var c4: int = b4.get_color_id(t4T)
	var rs4 = M20ReservationSeam.new(); rs4.bind(b4)
	var w4 = _m20_full(b4, null, rs4)
	var l4 = _m20_harness_bind(w4, _m20_slots([c4, c4, c4, c4, c4]))
	var r4T = l4.activate_slot(0, Vector2(-2.0, 11.5), 6.0)
	var r4U = l4.activate_slot(1, Vector2(26.0, 11.5), 6.0)
	rs4.mode = "identity_swap"; rs4.swap_from_owner = r4U.owner_id; rs4.swap_from_target = t4U; rs4.swap_to_target = t4V
	_m19_drive_to_arrival(r4T.agent)
	_check_eq(l4.get_last_outcome(), CompleteClearingLoop.Outcome.ROLLBACK_FAILED, "V08.9: identity-swap -> ROLLBACK_FAILED")
	rs4.mode = "normal"; l4 = null; _m20_teardown(w4)
	# reset during candidate phase; reset during reservation phase.
	for phase in ["candidate", "reservation"]:
		var bp = _m19_open_board_active(22, 22, [Vector2(11, 11)])
		var tp: int = bp.get_cell_index(11, 11); var cp: int = bp.get_color_id(tp)
		var wp
		var lp
		if phase == "candidate":
			var cseam = M20CandidateSeam.new(); cseam.bind(bp)
			wp = _m20_full(bp, cseam); lp = _m20_harness_bind(wp, _m20_slots([cp, cp, cp, cp, cp]))
			var rp = lp.activate_slot(0, Vector2(-2.0, 11.5), 6.0)
			cseam.sync_hook = func(_i): lp.reset()
			_m19_drive_to_arrival(rp.agent)
			cseam.sync_hook = Callable()
		else:
			var rseam = M20ReservationSeam.new(); rseam.bind(bp)
			wp = _m20_full(bp, null, rseam); lp = _m20_harness_bind(wp, _m20_slots([cp, cp, cp, cp, cp]))
			var rp = lp.activate_slot(0, Vector2(-2.0, 11.5), 6.0)
			rseam.resolve_hook = func(_t, _o): lp.reset()
			_m19_drive_to_arrival(rp.agent)
			rseam.resolve_hook = Callable()
		_check_eq(lp.get_last_outcome(), CompleteClearingLoop.Outcome.RESET_ABORTED, "V08.9: reset during %s phase -> RESET_ABORTED" % phase)
		_check_eq(bp.get_cell_state(tp), BoardState.CellState.ACTIVE, "V08.9: reset during %s -> target ACTIVE" % phase)
		lp = null; _m20_teardown(wp)
	# duplicate current arrival + distinct FIFO.
	var b5 = _m19_open_board_active(22, 22, [Vector2(9, 11), Vector2(13, 11)])
	var t5A: int = b5.get_cell_index(9, 11); var t5B: int = b5.get_cell_index(13, 11); var c5: int = b5.get_color_id(t5A)
	var cs5 = M20CandidateSeam.new(); cs5.bind(b5)
	var w5 = _m20_full(b5, cs5)
	var l5 = _m20_harness_bind(w5, _m20_slots([c5, c5, c5, c5, c5]))
	var r5A = l5.activate_slot(0, Vector2(-2.0, 11.5), 6.0)
	var r5B = l5.activate_slot(1, Vector2(26.0, 11.5), 6.0)
	cs5.sync_hook = func(_i):
		l5._on_assignment_arrived(r5A.owner_id, t5A, c5, r5A.agent) # duplicate current -> dropped
		_m19_drive_to_arrival(r5B.agent)                            # distinct -> FIFO
	_m19_drive_to_arrival(r5A.agent)
	_check_eq(l5.get_cleared_count(), 2, "V08.9: duplicate current dropped; distinct FIFO both cleared")
	cs5.sync_hook = Callable(); l5 = null; _m20_teardown(w5)
	# pair-narrow reset: original + unrelated; foreign-board + same-board wrong-owner preserved.
	var b6 = _m19_open_board_active(22, 22, [Vector2(11, 11), Vector2(15, 11)])
	var w6 = _m19_wire_real(b6)
	var r6 = w6["dispatcher"].dispatch(b6.get_color_id(b6.get_cell_index(11, 11)), Vector2(-2.0, 11.5), 6.0)
	var u6: int = b6.get_cell_index(15, 11)
	w6["reservations"].reserve(u6, 6001)
	w6["dispatcher"].reset()
	_check(w6["reservations"].get_owner(r6.target_index) == -1 and w6["reservations"].get_owner(u6) == 6001, "V08.9: pair-narrow releases pair, preserves unrelated")
	_m19_teardown(w6)
	var b7 = _m19_open_board_active(22, 22, [Vector2(11, 11)])
	var w7 = _m19_wire_real(b7)
	var r7 = w7["dispatcher"].dispatch(b7.get_color_id(b7.get_cell_index(11, 11)), Vector2(-2.0, 11.5), 6.0)
	var fb7 = _m19_open_board_active(22, 22, [Vector2(2, 2)])
	w7["reservations"].rebind(fb7)
	var fv7: int = fb7.get_cell_index(2, 2)
	w7["reservations"].reserve(fv7, r7.owner_id)
	w7["dispatcher"].reset()
	_check_eq(w7["reservations"].get_owner(fv7), r7.owner_id, "V08.9: foreign-board replacement survives reset")
	_m19_teardown(w7)
	# post-dispatch generation barrier (reset inside M19 dispatch) load-bearing.
	var wb = _m20_full_reset_access(_m19_open_board_active(22, 22, [Vector2(11, 11), Vector2(13, 11)]))
	var lb = _m20_loop(wb, _m20_slots([wb["board"].get_color_id(wb["board"].get_cell_index(11, 11)), 0, 0, 0, 0]))
	wb["select_access"].hook = func(): lb.reset()
	_check_eq(lb.activate_slot(0, Vector2(-2.0, 11.5), 6.0).failure_reason, DispatchResult.FailureReason.RESETTING, "V08.9: post-dispatch generation barrier -> RESETTING")
	wb["select_access"].hook = Callable(); lb = null; _m20_teardown(wb)

# ============================ M20-C001 V09 final exact-evidence gate ============
# Fresh, direct, EXACT-field evidence against the LOCKED V07/V08 production
# (blobs 06391839.../eee10149...). Validation-only. Frame-only cases live in
# tests/m20_v09_lifecycle_smoke.gd.

func _run_m20_v09_exact_evidence_tests() -> void:
	print("==== M20-C001 V09 exact-evidence validation ====")
	_v09_owner_claim()
	_v09_activation_snapshots()
	_v09_arrival_preflight()
	_v09_ledger()
	_v09_rollback()
	print("  M20 V09 exact-evidence validation complete")

# ---- exact snapshot helpers -------------------------------------------------
func _v09_rmap(w) -> Dictionary:
	var m := {}
	for idx in w["reservations"].get_reserved_indices():
		m[int(idx)] = w["reservations"].get_owner(int(idx))
	return m

func _v09_slot_tuples(slots) -> Array:
	var out := []
	for i in range(5):
		out.append([slots.get_slot_palette_id(i), slots.is_slot_available(i), slots.is_slot_active(i)])
	return out

func _v09_full_snap(w, slots) -> Dictionary:
	return {"cells": _snapshot_cell_states(w["board"]), "active": w["dispatcher"].get_active_count(),
		"next_owner": w["dispatcher"].peek_next_owner_id(), "rmap": _v09_rmap(w),
		"rcount": w["reservations"].get_reservation_count(), "slots": _v09_slot_tuples(slots)}

func _v09_dict_eq(a: Dictionary, b: Dictionary) -> bool:
	if a.size() != b.size():
		return false
	for k in a:
...
func _v09_snap_equal(w, slots, s: Dictionary, name: String) -> void:
	_check(_cell_states_equal(w["board"], s["cells"]), "%s: BoardState cells unchanged" % name)
	_check_eq(w["dispatcher"].get_active_count(), s["active"], "%s: active count unchanged" % name)
	_check_eq(w["dispatcher"].peek_next_owner_id(), s["next_owner"], "%s: next owner id unchanged" % name)
	_check_eq(w["reservations"].get_reservation_count(), s["rcount"], "%s: reservation count unchanged" % name)
	_check(_v09_dict_eq(_v09_rmap(w), s["rmap"]), "%s: reservation map unchanged" % name)
	_check(_v09_slot_tuples(slots) == s["slots"], "%s: five slot tuples unchanged" % name)

# ---- §3 owner claim exact lifecycle -----------------------------------------
func _v09_owner_claim() -> void:
	print("---- V09 §3: owner claim exact lifecycle ----")
	var board = _m19_open_board_active(22, 22, [Vector2(11, 11)])
	var color: int = board.get_color_id(board.get_cell_index(11, 11))
	var w = _m20_full(board)
	# Diagnostic listener BEFORE the first M20 bind.
	var diag := func(_o, _t, _c, _a): pass
	w["dispatcher"].assignment_arrived.connect(diag)
	var l1 = CompleteClearingLoop.new()
	_check(l1.bind(board, _m20_slots([color, color, color, color, color]), w["candidates"], w["reservations"], w["dispatcher"]), "V09.3: first loop binds with a pre-existing diagnostic observer")
	_check(w["dispatcher"].assignment_arrived.is_connected(diag), "V09.3: diagnostic observer remains connected/independent")
	var conn_with_owner: int = _m20_conn_count(w["dispatcher"]) # diag + owner cb = 2
	var l2 = CompleteClearingLoop.new()
	_check(not l2.bind(board, _m20_slots([color, color, color, color, color]), w["candidates"], w["reservations"], w["dispatcher"]), "V09.3: second different live loop bind fails")
	_check(not l2.is_bound(), "V09.3: second loop remains unbound")
	_check_eq(_m20_conn_count(w["dispatcher"]), conn_with_owner, "V09.3: second loop adds no M20 transaction callback")
	_check_eq(l2.get_cleared_count(), 0, "V09.3: second loop cleared_count zero")
	var before2 = _snapshot_cell_states(board)
	_check(not l2.activate_slot(0, Vector2(-2.0, 11.5)).success, "V09.3: second loop activation fails")
	l2.reset()
	_check(_cell_states_equal(board, before2) and w["dispatcher"].get_active_count() == 0, "V09.3: second loop reset mutates no canonical state")
	_check(l1.is_coherent(), "V09.3: first loop remains coherent")
	var r1 = _m20_activate_and_arrive(l1, 0, Vector2(-2.0, 11.5))
	_check(r1.success and l1.get_cleared_count() == 1, "V09.3: first loop one arrival -> exactly one clear")
	# owner-loop reset while alive: detached snapshot preserved, claim NOT released.
	var snap_before_reset = _snapshot_cell_states(board)
	l1.reset()
	_check(_cell_states_equal(board, snap_before_reset), "V09.3: owner reset preserves detached BoardState snapshot")
	var l3 = CompleteClearingLoop.new()
	_check(not l3.bind(board, _m20_slots([color, color, color, color, color]), w["candidates"], w["reservations"], w["dispatcher"]), "V09.3: different loop still rejected while owner alive (claim held through reset)")
	# first loop still usable after its reset.
	_check(l1.is_coherent(), "V09.3: first loop still usable after reset")
	w["dispatcher"].assignment_arrived.disconnect(diag)
	l1 = null; l3 = null
	print("  V09.3: owner-GC claim release + old-callback-absent + no-leak -> tests/m20_v09_lifecycle_smoke.gd")
	_m20_teardown(w)

# ---- §4 activation exact snapshot matrix ------------------------------------
func _v09_activation_snapshots() -> void:
	print("---- V09 §4: activation exact snapshot matrix ----")
	var board = _m19_open_board_active(22, 22, [Vector2(11, 11)])
	var color: int = board.get_color_id(board.get_cell_index(11, 11))
	var slots = _m20_slots([color, color, color, color, color])
	var w = _m20_full(board)
	...
	var cases: Array = [
		["slot -1", func(): return loop.activate_slot(-1, Vector2(-2.0, 11.5))],
		["slot 5", func(): return loop.activate_slot(5, Vector2(-2.0, 11.5))],
		["non-int slot", func(): return loop.activate_slot(2.7, Vector2(-2.0, 11.5))],
		["origin NaN x", func(): return loop.activate_slot(0, Vector2(NAN, 11.5))],
		["origin NaN y", func(): return loop.activate_slot(0, Vector2(11.5, NAN))],
		["origin +INF", func(): return loop.activate_slot(0, Vector2(INF, 11.5))],
		["origin -INF", func(): return loop.activate_slot(0, Vector2(-INF, 11.5))],
		...
		["speed -INF", func(): return loop.activate_slot(0, Vector2(-2.0, 11.5), -INF)],
		["speed 0", func(): return loop.activate_slot(0, Vector2(-2.0, 11.5), 0.0)],
		["speed negative", func(): return loop.activate_slot(0, Vector2(-2.0, 11.5), -4.0)],
	]
	for kase in cases:
		var s = _v09_full_snap(w, slots)
		_check(not kase[1].call().success, "V09.4: %s rejected" % kase[0])
		_v09_snap_equal(w, slots, s, "V09.4 " + kase[0])
	# unavailable slot (fresh) with full snapshot.
	var su = _m20_slots([color, color, color, color, color]); su.set_slot_available(0, false)
	var wu = _m20_full(_m19_open_board_active(22, 22, [Vector2(11, 11)]))
	var lu = _m20_loop(wu, su)
	var sn = _v09_full_snap(wu, su)
	_check(not lu.activate_slot(0, Vector2(-2.0, 11.5)).success, "V09.4: unavailable slot rejected")
	_v09_snap_equal(wu, su, sn, "V09.4 unavailable slot")
	lu = null; _m20_teardown(wu)
	loop = null; _m20_teardown(w)
	# absent color / no target (NO_REACHABLE_TARGET) exact snapshot.
	var bnt = _m19_open_board_active(22, 22, [Vector2(10, 2)])
	var snt = _m20_slots([(bnt.get_color_id(bnt.get_cell_index(10, 2)) + 1) % 5, 0, 0, 0, 0])
	var wnt = _m20_full(bnt)
	var lnt = _m20_loop(wnt, snt)
	var snt_s = _v09_full_snap(wnt, snt)
	_check_eq(lnt.activate_slot(0, Vector2(-2.0, 2.5)).failure_reason, DispatchResult.FailureReason.NO_REACHABLE_TARGET, "V09.4: absent color -> NO_REACHABLE_TARGET")
	_v09_snap_equal(wnt, snt, snt_s, "V09.4 absent color")
	lnt = null; _m20_teardown(wnt)
	# fully enclosed matching target exact snapshot.
	var be = _m19_enclosed_sole_candidate_board()
	var se = _m20_slots([1, 1, 1, 1, 1])
	var we = _m20_full(be)
	var le = _m20_loop(we, se)
	var se_s = _v09_full_snap(we, se)
	_check_eq(le.activate_slot(0, Vector2(-2.0, 2.5)).failure_reason, DispatchResult.FailureReason.NO_REACHABLE_TARGET, "V09.4: enclosed -> NO_REACHABLE_TARGET")
	_v09_snap_equal(we, se, se_s, "V09.4 enclosed")
	le = null; _m20_teardown(we)
	# nested activation -> REENTRANT, no second reservation/owner.
	var seam = M20CandidateSeam.new()
	var bn = _m19_open_board_active(22, 22, [Vector2(11, 11), Vector2(13, 11)]); seam.bind(bn)
	var wn = _m20_full(bn, seam)
	var ln = _m20_harness_bind(wn, _m20_slots([bn.get_color_id(bn.get_cell_index(11, 11)), 0, 0, 0, 0]))
	var reason := [&""]; var owner_before: int = wn["dispatcher"].peek_next_owner_id()
	seam.coherence_hook = func(): reason[0] = ln.activate_slot(1, Vector2(26.0, 11.5)).failure_reason
	var outer = ln.activate_slot(0, Vector2(-2.0, 11.5), 6.0)
	_check_eq(reason[0], DispatchResult.FailureReason.REENTRANT, "V09.4: nested activation -> REENTRANT")
	_check_eq(wn["dispatcher"].get_active_count(), 1, "V09.4: nested created no second agent")
	_check_eq(wn["reservations"].get_reservation_count(), 1, "V09.4: nested created no second reservation")
	seam.coherence_hook = Callable(); ln = null; _m20_teardown(wn)
	# reset during activation preflight -> RESETTING.
	var seam2 = M20CandidateSeam.new()
	var bp = _m19_open_board_active(22, 22, [Vector2(11, 11)]); seam2.bind(bp)
	var wp = _m20_full(bp, seam2)
	var lp = _m20_harness_bind(wp, _m20_slots([bp.get_color_id(bp.get_cell_index(11, 11)), 0, 0, 0, 0]))
	seam2.coherence_hook = func(): lp.reset()
	var op: int = wp["dispatcher"].peek_next_owner_id()
	var rp = lp.activate_slot(0, Vector2(-2.0, 11.5), 6.0)
	_check_eq(rp.failure_reason, DispatchResult.FailureReason.RESETTING, "V09.4: reset during activation preflight -> RESETTING")
	_check(wp["dispatcher"].peek_next_owner_id() >= op, "V09.4: preflight-reset owner id monotonic (no reuse)")
	seam2.coherence_hook = Callable(); lp = null; _m20_teardown(wp)
	# reset inside M19 dispatch -> RESETTING, monotonic owner consume.
	var wr = _m20_full_reset_access(_m19_open_board_active(22, 22, [Vector2(11, 11), Vector2(13, 11)]))
	var lr = _m20_loop(wr, _m20_slots([wr["board"].get_color_id(wr["board"].get_cell_index(11, 11)), 0, 0, 0, 0]))
	var ob: int = wr["dispatcher"].peek_next_owner_id()
	wr["select_access"].hook = func(): lr.reset()
	var rr = lr.activate_slot(0, Vector2(-2.0, 11.5), 6.0)
	_check_eq(rr.failure_reason, DispatchResult.FailureReason.RESETTING, "V09.4: reset inside M19 dispatch -> RESETTING (never raw SUCCESS)")
	_check(wr["dispatcher"].peek_next_owner_id() == ob + 1, "V09.4: reset-inside-dispatch consumed exactly one owner (monotonic)")
	_check_eq(wr["dispatcher"].get_active_count(), 0, "V09.4: deterministic cleanup, no active assignment")
	wr["select_access"].hook = Callable(); lr = null; _m20_teardown(wr)
	# post-dispatch M20 coherence loss -> COHERENCE_FAILED + cleanup.
	var wc = _m20_full_reset_access(_m19_open_board_active(22, 22, [Vector2(11, 11)]), true)
	var lc = _m20_loop(wc, _m20_slots([wc["board"].get_color_id(wc["board"].get_cell_index(11, 11)), 0, 0, 0, 0]))
	var rend = wc["renderer"]
	wc["select_access"].hook = func(): rend.queue_free()
	var rc = lc.activate_slot(0, Vector2(-2.0, 11.5), 6.0)
	_check_eq(rc.failure_reason, DispatchResult.FailureReason.COHERENCE_FAILED, "V09.4: post-dispatch coherence loss -> COHERENCE_FAILED")
	_check(wc["dispatcher"].get_active_count() == 0 and wc["reservations"].get_reservation_count() == 0, "V09.4: coherence-loss deterministic cleanup")
	wc["select_access"].hook = Callable()
	if is_instance_valid(rend): root.remove_child(rend); rend.free()
	wc["renderer"] = null; lc = null; _m20_teardown(wc)

# ---- §5 arrival-preflight exact invariant matrix ----------------------------
## Fresh ARRIVED-but-unconsumed assignment on a board with an UNRELATED sentinel
## ACTIVE cell (13,11) that also holds an unrelated reservation for owner 9090.
func _v09_arrived_sentinel(w) -> Dictionary:
	var target: int = w["board"].get_cell_index(11, 11)
	var color: int = w["board"].get_color_id(target)
	var loop = _m20_loop(w, _m20_slots([color, color, color, color, color]))
	var r = loop.activate_slot(0, Vector2(-2.0, 11.5), 6.0)
	var sentinel: int = w["board"].get_cell_index(13, 11)
	w["reservations"].reserve(sentinel, 9090)
	w["dispatcher"].assignment_arrived.disconnect(loop._arrival_cb)
	_m19_drive_to_arrival(r.agent)
	return {"loop": loop, "owner": r.owner_id, "target": target, "color": color, "agent": r.agent, "sentinel": sentinel}

func _v09_arrival_preflight() -> void:
	print("---- V09 §5: arrival-preflight exact invariants ----")
	# identity variants, fresh arrangement each, exact invariants.
	var variants: Array = [["wrong owner", 1, 0, 0, false], ["wrong target", 0, 1, 0, false],
		["wrong color", 0, 0, 1, false], ["wrong agent", 0, 0, 0, true], ["unknown owner", 9999, 0, 0, false]]
	for v in variants:
		var w = _m20_full(_m19_open_board_active(22, 22, [Vector2(11, 11), Vector2(13, 11)]))
		var a = _v09_arrived_sentinel(w)
		var before = _snapshot_cell_states(w["board"])
		var ow = (9999 if v[0] == "unknown owner" else a["owner"] + int(v[1]))
		var tg = a["target"] + int(v[2])
		var cl = a["color"] + int(v[3])
		var ag = (null if v[4] else a["agent"])
		a["loop"]._on_assignment_arrived(ow, tg, cl, ag)
		_check_eq(a["loop"].get_last_outcome(), CompleteClearingLoop.Outcome.PREFLIGHT_REJECTED, "V09.5 %s: PREFLIGHT_REJECTED" % v[0])
		_check(_cell_states_equal(w["board"], before), "V09.5 %s: BoardState snapshot unchanged" % v[0])
		_check_eq(a["loop"].get_cleared_count(), 0, "V09.5 %s: cleared_count unchanged" % v[0])
		_check_eq(w["reservations"].get_owner(a["target"]), a["owner"], "V09.5 %s: reservation target->owner intact" % v[0])
		_check_eq(w["reservations"].get_target_for_owner(a["owner"]), a["target"], "V09.5 %s: reservation owner->target intact" % v[0])
		_check(w["dispatcher"].has_owner(a["owner"]), "V09.5 %s: dispatcher assignment pending" % v[0])
		_check(w["candidates"].get_candidates(a["color"], null).has(a["target"]), "V09.5 %s: raw target candidate present" % v[0])
		_check_eq(w["reservations"].get_owner(a["sentinel"]), 9090, "V09.5 %s: unrelated sentinel reservation unchanged" % v[0])
		a["loop"] = null; _m20_teardown(w)

	# missing reservation.
	var wm = _m20_full(_m19_open_board_active(22, 22, [Vector2(11, 11), Vector2(13, 11)]))
	var am = _v09_arrived_sentinel(wm)
	wm["reservations"].release_for_owner(am["owner"])
	am["loop"]._on_assignment_arrived(am["owner"], am["target"], am["color"], am["agent"])
	_check_eq(am["loop"].get_last_outcome(), CompleteClearingLoop.Outcome.PREFLIGHT_REJECTED, "V09.5 missing reservation: rejected")
	_check_eq(wm["board"].get_cell_state(am["target"]), BoardState.CellState.ACTIVE, "V09.5 missing reservation: target ACTIVE")
	am["loop"] = null; _m20_teardown(wm)

	# same-board T->O2: replacement survives BEFORE reset and after pair-narrow reset.
	var w2 = _m20_full(_m19_open_board_active(22, 22, [Vector2(11, 11), Vector2(13, 11)]))
	var a2 = _v09_arrived_sentinel(w2)
	w2["reservations"].release(a2["target"], a2["owner"])
	w2["reservations"].reserve(a2["target"], 7070)
	a2["loop"]._on_assignment_arrived(a2["owner"], a2["target"], a2["color"], a2["agent"])
	_check_eq(a2["loop"].get_last_outcome(), CompleteClearingLoop.Outcome.PREFLIGHT_REJECTED, "V09.5 T->O2: original arrival rejected")
	_check_eq(w2["reservations"].get_owner(a2["target"]), 7070, "V09.5 T->O2: replacement survives failed preflight (before reset)")
	w2["dispatcher"].assignment_arrived.connect(a2["loop"]._arrival_cb)
	a2["loop"].reset()
	_check_eq(w2["reservations"].get_owner(a2["target"]), 7070, "V09.5 T->O2: replacement survives pair-narrow reset")
	a2["loop"] = null; _m20_teardown(w2)

	# reservations rebind(foreign) + foreign replacement.
	var w3 = _m20_full(_m19_open_board_active(22, 22, [Vector2(11, 11)]))
	var a3 = _v09_arrived_sentinel_nores(w3)
	var fb = _m19_open_board_active(22, 22, [Vector2(4, 4)])
	w3["reservations"].rebind(fb)
	var fv: int = fb.get_cell_index(4, 4)
	w3["reservations"].reserve(fv, a3["owner"])
	a3["loop"]._on_assignment_arrived(a3["owner"], a3["target"], a3["color"], a3["agent"])
	_check_eq(a3["loop"].get_last_outcome(), CompleteClearingLoop.Outcome.PREFLIGHT_REJECTED, "V09.5 foreign reservation: rejected")
	_check_eq(w3["reservations"].get_owner(fv), a3["owner"], "V09.5 foreign reservation: survives failed preflight")
	w3["dispatcher"].assignment_arrived.connect(a3["loop"]._arrival_cb)
	a3["loop"].reset()
	_check_eq(w3["reservations"].get_owner(fv), a3["owner"], "V09.5 foreign reservation: survives pair-narrow reset")
	a3["loop"] = null; _m20_teardown(w3)

	# candidate rebind(foreign): reservation+dispatcher held, cleared 0.
	var w4 = _m20_full(_m19_open_board_active(22, 22, [Vector2(11, 11)]))
	var a4 = _v09_arrived_sentinel_nores(w4)
	w4["candidates"].rebind(_m19_open_board_active(22, 22, [Vector2(2, 2)]))
	a4["loop"]._on_assignment_arrived(a4["owner"], a4["target"], a4["color"], a4["agent"])
	_check_eq(a4["loop"].get_last_outcome(), CompleteClearingLoop.Outcome.PREFLIGHT_REJECTED, "V09.5 candidate foreign: rejected")
	_check(w4["reservations"].get_owner(a4["target"]) == a4["owner"] and w4["dispatcher"].has_owner(a4["owner"]) and a4["loop"].get_cleared_count() == 0, "V09.5 candidate foreign: reservation+assignment held, cleared 0")
	a4["loop"] = null; _m20_teardown(w4)

	# candidate rebind(null).
	var w5 = _m20_full(_m19_open_board_active(22, 22, [Vector2(11, 11)]))
	var a5 = _v09_arrived_sentinel_nores(w5)
	w5["candidates"].rebind(null)
	a5["loop"]._on_assignment_arrived(a5["owner"], a5["target"], a5["color"], a5["agent"])
	_check_eq(a5["loop"].get_last_outcome(), CompleteClearingLoop.Outcome.PREFLIGHT_REJECTED, "V09.5 candidate rebind(null): rejected cleanly")
	a5["loop"] = null; _m20_teardown(w5)

	# target externally CLEARED.
	var w6 = _m20_full(_m19_open_board_active(22, 22, [Vector2(11, 11)]))
	var a6 = _v09_arrived_sentinel_nores(w6)
	w6["board"].set_cell_state(a6["target"], BoardState.CellState.CLEARED)
	a6["loop"]._on_assignment_arrived(a6["owner"], a6["target"], a6["color"], a6["agent"])
	_check_eq(a6["loop"].get_last_outcome(), CompleteClearingLoop.Outcome.PREFLIGHT_REJECTED, "V09.5 externally CLEARED: rejected")
	_check_eq(a6["loop"].get_cleared_count(), 0, "V09.5 externally CLEARED: cleared_count unchanged")
	a6["loop"] = null; _m20_teardown(w6)

	# configured renderer rebound foreign AFTER assignment, BEFORE arrival -> no clear.
	var w7 = _m20_full(_m19_open_board_active(22, 22, [Vector2(11, 11)]), null, null, true)
	var t7: int = w7["board"].get_cell_index(11, 11); var c7: int = w7["board"].get_color_id(t7)
	var l7 = _m20_loop(w7, _m20_slots([c7, c7, c7, c7, c7]))
	var r7 = l7.activate_slot(0, Vector2(-2.0, 11.5), 6.0)
	w7["renderer"].configure(_m19_open_board_active(22, 22, [Vector2(2, 2)]), PackedStringArray(BoardDebugFixturesM20.PALETTE), Vector2(90, 90))
	_m19_drive_to_arrival(r7.agent)
	_check_eq(w7["board"].get_cell_state(t7), BoardState.CellState.ACTIVE, "V09.5 renderer foreign-board: no clear")
	_check(w7["reservations"].get_owner(t7) == r7.owner_id and w7["dispatcher"].has_owner(r7.owner_id), "V09.5 renderer foreign-board: reservation+assignment held")
	l7 = null; root.remove_child(w7["renderer"]); w7["renderer"].free(); w7["renderer"] = null; _m20_teardown(w7)

	# configured renderer queued before arrival -> no clear.
	var w8 = _m20_full(_m19_open_board_active(22, 22, [Vector2(11, 11)]), null, null, true)
	var t8: int = w8["board"].get_cell_index(11, 11); var c8: int = w8["board"].get_color_id(t8)
	var l8 = _m20_loop(w8, _m20_slots([c8, c8, c8, c8, c8]))
	var r8 = l8.activate_slot(0, Vector2(-2.0, 11.5), 6.0)
	w8["renderer"].queue_free()
	_m19_drive_to_arrival(r8.agent)
	_check(w8["board"].get_cell_state(t8) == BoardState.CellState.ACTIVE and l8.get_cleared_count() == 0, "V09.5 renderer queued: no clear")
	if is_instance_valid(w8["renderer"]): root.remove_child(w8["renderer"]); w8["renderer"].free()
	w8["renderer"] = null; l8 = null; _m20_teardown(w8)

	# stale replay after reset.
	var w9 = _m20_full(_m19_open_board_active(22, 22, [Vector2(11, 11)]))
	var a9 = _v09_arrived_sentinel_nores(w9)
	w9["dispatcher"].assignment_arrived.connect(a9["loop"]._arrival_cb)
	a9["loop"].reset()
	a9["loop"]._on_assignment_arrived(a9["owner"], a9["target"], a9["color"], a9["agent"])
	_check_eq(a9["loop"].get_cleared_count(), 0, "V09.5 stale replay after reset: cannot clear")
	a9["loop"] = null; _m20_teardown(w9)

	# duplicate current owner+agent (dropped, single clear) + distinct FIFO.
	var seam = M20CandidateSeam.new()
	var bd = _m19_open_board_active(22, 22, [Vector2(9, 11), Vector2(13, 11)]); seam.bind(bd)
	var wd = _m20_full(bd, seam)
	var ld = _m20_harness_bind(wd, _m20_slots([bd.get_color_id(bd.get_cell_index(9, 11)), bd.get_color_id(bd.get_cell_index(9, 11)), 0, 0, 0]))
	var rA = ld.activate_slot(0, Vector2(-2.0, 11.5), 6.0)
	var rB = ld.activate_slot(1, Vector2(26.0, 11.5), 6.0)
	seam.sync_hook = func(_i):
		ld._on_assignment_arrived(rA.owner_id, bd.get_cell_index(9, 11), bd.get_color_id(bd.get_cell_index(9, 11)), rA.agent) # dup current -> dropped
		_m19_drive_to_arrival(rB.agent)                                                                                    # distinct -> FIFO
	_m19_drive_to_arrival(rA.agent)
	_check_eq(ld.get_cleared_count(), 2, "V09.5 dup-current dropped + distinct FIFO both cleared")
	seam.sync_hook = Callable(); ld = null; _m20_teardown(wd)

	# duplicate ALREADY-QUEUED owner+agent as its own test. B is driven to the
	# authenticated ARRIVED state while the loop is disconnected (so it is pending-
	# unconsumed), then during A's transaction B is enqueued TWICE; the second
	# (already-queued) enqueue must be dropped, so B still clears exactly once.
	var seam2 = M20CandidateSeam.new()
	var bq = _m19_open_board_active(22, 22, [Vector2(9, 11), Vector2(13, 11)]); seam2.bind(bq)
	var wq2 = _m20_full(bq, seam2)
	var qcol: int = bq.get_color_id(bq.get_cell_index(9, 11))
	var lq = _m20_harness_bind(wq2, _m20_slots([qcol, qcol, 0, 0, 0]))
	var qA = lq.activate_slot(0, Vector2(-2.0, 11.5), 6.0)
	var qB = lq.activate_slot(1, Vector2(26.0, 11.5), 6.0)
	var qBt: int = bq.get_cell_index(13, 11)
	wq2["dispatcher"].assignment_arrived.disconnect(lq._arrival_cb)
	_m19_drive_to_arrival(qB.agent) # B now authenticated-arrived, unconsumed
	wq2["dispatcher"].assignment_arrived.connect(lq._arrival_cb)
	seam2.sync_hook = func(_i):
		lq._on_assignment_arrived(qB.owner_id, qBt, qcol, qB.agent)
		lq._on_assignment_arrived(qB.owner_id, qBt, qcol, qB.agent)
	_m19_drive_to_arrival(qA.agent)
	_check_eq(lq.get_cleared_count(), 2, "V09.5 duplicate already-queued dropped (A + one B only)")
	seam2.sync_hook = Callable(); lq = null; _m20_teardown(wq2)
	print("  V09.5: frame-aware truly-freed configured renderer before arrival -> tests/m20_v09_lifecycle_smoke.gd")

## Arrived-but-unconsumed with NO extra sentinel reservation.
func _v09_arrived_sentinel_nores(w) -> Dictionary:
	var target: int = w["board"].get_cell_index(11, 11)
	var color: int = w["board"].get_color_id(target)
	var loop = _m20_loop(w, _m20_slots([color, color, color, color, color]))
	var r = loop.activate_slot(0, Vector2(-2.0, 11.5), 6.0)
	w["dispatcher"].assignment_arrived.disconnect(loop._arrival_cb)
	_m19_drive_to_arrival(r.agent)
	return {"loop": loop, "owner": r.owner_id, "target": target, "color": color, "agent": r.agent}

# ---- §6 exact final SB-M20 ledger -------------------------------------------
func _v09_ledger() -> void:
	print("---- V09 §6: exact SB-M20-001..014 ledger ----")
	# 001.
	var b1 = _m19_open_board_active(24, 24, [Vector2(12, 12)])
	var t1: int = b1.get_cell_index(12, 12); var c1: int = b1.get_color_id(t1)
	var w1 = _m20_full(b1, null, null, true)
	...
	var r1 = _m20_activate_and_arrive(l1, 0, Vector2(-2.0, 12.5))
	var p1: Vector2i = b1.get_cell_position(t1)
	_check(b1.get_cell_state(t1) == BoardState.CellState.CLEARED, "SB-M20-001: CLEARED")
	_check(not w1["candidates"].get_candidates(c1, null).has(t1), "SB-M20-001: absent raw candidate")
	_check_eq(w1["routing_access"].classify_cell(p1.x, p1.y, -1), ProductionAccessQuery.CellClass.OPEN, "SB-M20-001: AccessQuery OPEN")
	_check_eq(w1["reservations"].get_owner(t1), -1, "SB-M20-001: reservation target->owner absent")
	_check_eq(w1["reservations"].get_target_for_owner(r1.owner_id), -1, "SB-M20-001: reservation owner->target absent")
	_check(not w1["dispatcher"].has_owner(r1.owner_id), "SB-M20-001: dispatcher owner absent")
	_check(r1.agent.is_queued_for_deletion(), "SB-M20-001: agent queued for deletion")
	_check(_colors_close(w1["renderer"].get_pixel_color(p1.x, p1.y), Color(0, 0, 0, 0), 0.02), "SB-M20-001: renderer alpha 0")
	l1 = null; _m20_teardown(w1)
	# 002 no-target + enclosed, exact.
	for kase in [["no-target", _m19_open_board_active(22, 22, [Vector2(10, 2)]), 2.5, true], ["enclosed", _m19_enclosed_sole_candidate_board(), 2.5, false]]:
		var bk = kase[1]
		var slk = _m20_slots([(1 if not kase[3] else (bk.get_color_id(bk.get_cell_index(10, 2)) + 1) % 5), 0, 0, 0, 0])
		var wk = _m20_full(bk)
		var lk = _m20_loop(wk, slk)
		var sk = _v09_full_snap(wk, slk)
		_check_eq(lk.activate_slot(0, Vector2(-2.0, kase[2])).failure_reason, DispatchResult.FailureReason.NO_REACHABLE_TARGET, "SB-M20-002 %s: NO_REACHABLE_TARGET" % kase[0])
		_v09_snap_equal(wk, slk, sk, "SB-M20-002 " + kase[0])
		lk = null; _m20_teardown(wk)
	print("  SB-M20-003: frame destruction/no-orphan -> tests/m20_queue_free_smoke.gd (run separately)")
	# 004 1x1.
	var b4 = _m19_open_board_active(1, 1, [Vector2(0, 0)])
	var w4 = _m20_full(b4); var l4 = _m20_loop(w4, _m20_slots([b4.get_color_id(0), 0, 0, 0, 0]))
	_check(_m20_activate_and_arrive(l4, 0, Vector2(-2.0, 0.5)).success and l4.get_cleared_count() == 1, "SB-M20-004: 1x1 clears")
	_check_eq(l4.activate_slot(0, Vector2(-2.0, 0.5)).failure_reason, DispatchResult.FailureReason.NO_REACHABLE_TARGET, "SB-M20-004: exhausted")
	l4 = null; _m20_teardown(w4)
	# 005 one-color exhaustion.
	var cells5: Array = []
	for x in range(8): cells5.append(Vector2(x + 2, 6))
	var b5 = _m19_open_board_active(24, 24, cells5)
	var c5: int = b5.get_color_id(b5.get_cell_index(2, 6))
	var w5 = _m20_full(b5); var l5 = _m20_loop(w5, _m20_slots([c5, c5, c5, c5, c5]))
	var n5 := 0
	for _i in range(20):
		if not _m20_activate_and_arrive(l5, 0, Vector2(-2.0, 6.5)).success: break
		n5 += 1
	_check(n5 == 8 and b5.count_cells_by_state(BoardState.CellState.ACTIVE) == 0, "SB-M20-005: one-color exact exhaustion (8)")
	l5 = null; _m20_teardown(w5)
	# 006 multi-color; other-color bucket unchanged after the relevant clear.
	var b6 = _m19_open_board_active(24, 24, [Vector2(12, 0), Vector2(12, 1), Vector2(14, 3)])
	var t6a: int = b6.get_cell_index(12, 0); var c6a: int = b6.get_color_id(t6a)
	var c6other: int = b6.get_color_id(b6.get_cell_index(14, 3))
	var w6 = _m20_full(b6)
	var l6 = _m20_loop(w6, _m20_slots([c6a, b6.get_color_id(b6.get_cell_index(12, 1)), 0, 0, 0]))
	var other_before: Array = w6["candidates"].get_candidates(c6other, null)
	_check(_m20_activate_and_arrive(l6, 0, Vector2(-2.0, 0.5)).target_index == t6a, "SB-M20-006: slot0 clears its color")
	_check_eq(w6["candidates"].get_candidates(c6other, null), other_before, "SB-M20-006: unrelated other-color bucket unchanged")
	l6 = null; _m20_teardown(w6)
	# 007 five in-flight, both-direction pairs, first preserves four.
	var b7 = _m19_open_board_active(24, 24, [Vector2(12, 0), Vector2(12, 1), Vector2(12, 2), Vector2(12, 3), Vector2(12, 4)])
	var pal7: Array = []
	for y in range(5): pal7.append(b7.get_color_id(b7.get_cell_index(12, y)))
	var w7 = _m20_full(b7); var l7 = _m20_loop(w7, _m20_slots(pal7))
	...
	var both7 := true
	for r in res7:
		if w7["reservations"].get_owner(r.target_index) != r.owner_id or w7["reservations"].get_target_for_owner(r.owner_id) != r.target_index: both7 = false
	_check(res7.size() == 5 and ow.size() == 5 and tg.size() == 5 and both7, "SB-M20-007: five unique owners/targets, exact both-direction pairs")
	_m19_drive_to_arrival(res7[0].agent)
	_check_eq(w7["reservations"].get_owner(res7[0].target_index), -1, "SB-M20-007: first arrival removes only its pair")
	var four7 := true
	for i in range(1, 5):
		if w7["reservations"].get_owner(res7[i].target_index) != res7[i].owner_id or w7["reservations"].get_target_for_owner(res7[i].owner_id) != res7[i].target_index: four7 = false
	_check(four7, "SB-M20-007: other four exact both directions preserved")
	for i in range(1, 5): _m19_drive_to_arrival(res7[i].agent)
	_check(l7.get_cleared_count() == 5 and w7["dispatcher"].get_active_count() == 0 and w7["reservations"].get_reservation_count() == 0, "SB-M20-007: all five finalize, active 0, reservations 0")
	l7 = null; _m20_teardown(w7)
	# 008-013 dims.
	for d in [[24, 24, "008 Easy", 12], [34, 34, "009 Medium", 15], [44, 44, "010 Hard", 20], [54, 54, "011 VeryHard", 25], [59, 59, "012 59x59", 29], [53, 59, "013 rect", 20]]:
		var bd = _m19_open_board_active(d[0], d[1], [Vector2(d[3], 10)])
		var td: int = bd.get_cell_index(d[3], 10)
		var wd = _m20_full(bd); var ldd = _m20_loop(wd, _m20_slots([bd.get_color_id(td), 0, 0, 0, 0]))
		var cbefore: int = bd.count_cells_by_state(BoardState.CellState.CLEARED)
		_check(_m20_activate_and_arrive(ldd, 0, Vector2(-2.0, 10.5)).success, "SB-M20-%s clears" % d[2])
		_check_eq(bd.count_cells_by_state(BoardState.CellState.CLEARED), cbefore + 1, "SB-M20-%s exactly one target changed (single-cell path)" % d[2])
		ldd = null; _m20_teardown(wd)
	# 014 AL-028 exact + rapid + reset-multiple + slot tuple snapshots.
	var bA = _m19_open_board_active(8, 8, [Vector2(1, 2), Vector2(2, 2), Vector2(3, 2), Vector2(2, 1), Vector2(2, 3)])
	var door: int = bA.get_cell_index(1, 2); var inner: int = bA.get_cell_index(2, 2)
	var wA = _m20_full(bA)
	var probe = ProductionTargetAccess.new(wA["routing"], wA["routing_access"], bA)
	probe.set_origin(Vector2(-2.0, 2.5))
	_check(not probe.is_targetable(inner), "SB-M20-014: B NOT reachable while ACTIVE A blocks it")
	var slotsA = _m20_slots([bA.get_color_id(door), bA.get_color_id(door), bA.get_color_id(door), bA.get_color_id(door), bA.get_color_id(door)])
	var lA = _m20_loop(wA, slotsA)
	var slot_before: Array = _v09_slot_tuples(slotsA)
	_check(_m20_activate_and_arrive(lA, 0, Vector2(-2.0, 2.5)).target_index == door, "SB-M20-014: first activation clears A")
	var rB = lA.activate_slot(1, Vector2(-2.0, 2.5), 6.0)
	_check(rB.success and rB.target_index == inner, "SB-M20-014: second activation selects B")
	_check(wA["reservations"].get_owner(inner) == rB.owner_id and wA["reservations"].get_target_for_owner(rB.owner_id) == inner, "SB-M20-014: exact B reservation both directions before arrival")
	_m19_drive_to_arrival(rB.agent)
	_check_eq(bA.get_cell_state(inner), BoardState.CellState.CLEARED, "SB-M20-014: B clears/finalizes")
	_check(_v09_slot_tuples(slotsA) == slot_before, "SB-M20-014: slot tuples unchanged by ordinary successes")
	lA = null; _m20_teardown(wA)
	# rapid >=25.
	var rc: Array = []
	for x in range(28): rc.append(Vector2(x + 1, 6))
	...
	_check(cyc >= 25 and lrap.get_cleared_count() == 28, "SB-M20-014: rapid >=25 cycles (%d)" % cyc)
	lrap = null; _m20_teardown(wrap)
	# reset with multiple in-flight preserves BoardState + raw candidate truth.
	var bm = _m19_open_board_active(24, 24, [Vector2(12, 0), Vector2(12, 1), Vector2(12, 2)])
	var wm = _m20_full(bm)
	var lm = _m20_loop(wm, _m20_slots([bm.get_color_id(bm.get_cell_index(12, 0)), bm.get_color_id(bm.get_cell_index(12, 1)), bm.get_color_id(bm.get_cell_index(12, 2)), 0, 0]))
	for i in range(3): lm.activate_slot(i, Vector2(-2.0, float(i) + 0.5), 6.0)
	var cells_before = _snapshot_cell_states(bm)
	var cand_before: Array = wm["candidates"].get_candidates(bm.get_color_id(bm.get_cell_index(12, 0)), null)
	lm.reset()
	_check(wm["dispatcher"].get_active_count() == 0 and wm["reservations"].get_reservation_count() == 0, "SB-M20-014: reset cleans active+reservations")
	_check(_cell_states_equal(bm, cells_before), "SB-M20-014: reset preserves BoardState")
	_check_eq(wm["candidates"].get_candidates(bm.get_color_id(bm.get_cell_index(12, 0)), null), cand_before, "SB-M20-014: reset preserves raw candidate truth")
	lm = null; _m20_teardown(wm)

# ---- §7 exact rollback/reset/contention matrix ------------------------------
func _v09_rollback() -> void:
	print("---- V09 §7: exact rollback/reset/contention ----")
	# candidate mutate-before-false.
	var b1 = _m19_open_board_active(22, 22, [Vector2(11, 11)]); var t1: int = b1.get_cell_index(11, 11); var c1: int = b1.get_color_id(t1)
	var cs1 = M20CandidateSeam.new(); cs1.bind(b1)
	var w1 = _m20_full(b1, cs1); var l1 = _m20_harness_bind(w1, _m20_slots([c1, c1, c1, c1, c1]))
	var r1 = l1.activate_slot(0, Vector2(-2.0, 11.5), 6.0); cs1.mode = "mutate_false"
	_m19_drive_to_arrival(r1.agent)
	_check(l1.get_last_outcome() == CompleteClearingLoop.Outcome.CANDIDATE_ROLLBACK and b1.get_cell_state(t1) == BoardState.CellState.ACTIVE and w1["candidates"].get_candidates(c1, null).has(t1) and w1["reservations"].get_owner(t1) == r1.owner_id and w1["dispatcher"].has_owner(r1.owner_id), "V09.7: candidate mutate-before-false -> exact tuple restored")
	cs1.mode = "normal"; l1 = null; _m20_teardown(w1)
	# reservation mutate-before-false.
	var b2 = _m19_open_board_active(22, 22, [Vector2(11, 11)]); var t2: int = b2.get_cell_index(11, 11); var c2: int = b2.get_color_id(t2)
	var rs2 = M20ReservationSeam.new(); rs2.bind(b2)
	var w2 = _m20_full(b2, null, rs2); var l2 = _m20_harness_bind(w2, _m20_slots([c2, c2, c2, c2, c2]))
	var r2 = l2.activate_slot(0, Vector2(-2.0, 11.5), 6.0); rs2.mode = "mutate_false"
	_m19_drive_to_arrival(r2.agent)
	_check(l2.get_last_outcome() == CompleteClearingLoop.Outcome.RESERVATION_ROLLBACK and w2["reservations"].get_owner(t2) == r2.owner_id and w2["reservations"].get_target_for_owner(r2.owner_id) == t2, "V09.7: reservation mutate-before-false -> exact map restored")
	rs2.mode = "normal"; l2 = null; _m20_teardown(w2)
	# candidate + reservation true-without-postcondition.
	for kind in ["candidate", "reservation"]:
		var bk = _m19_open_board_active(22, 22, [Vector2(11, 11)]); var tk: int = bk.get_cell_index(11, 11); var ck: int = bk.get_color_id(tk)
		var wk; var lk; var seam
		if kind == "candidate":
			seam = M20CandidateSeam.new(); seam.bind(bk); wk = _m20_full(bk, seam)
		else:
			seam = M20ReservationSeam.new(); seam.bind(bk); wk = _m20_full(bk, null, seam)
		lk = _m20_harness_bind(wk, _m20_slots([ck, ck, ck, ck, ck]))
		var rk = lk.activate_slot(0, Vector2(-2.0, 11.5), 6.0); seam.mode = "true_noop"
		_m19_drive_to_arrival(rk.agent)
		var exp = CompleteClearingLoop.Outcome.CANDIDATE_ROLLBACK if kind == "candidate" else CompleteClearingLoop.Outcome.RESERVATION_ROLLBACK
		_check_eq(lk.get_last_outcome(), exp, "V09.7: %s true-without-postcondition -> rollback" % kind)
		seam.mode = "normal"; lk = null; _m20_teardown(wk)
	# unrelated same-color candidate loss -> restored (not silent).
	var b3 = _m19_open_board_active(22, 22, [Vector2(9, 11), Vector2(11, 11)]); var t3T: int = b3.get_cell_index(9, 11); var t3U: int = b3.get_cell_index(11, 11); var c3: int = b3.get_color_id(t3T)
	var cs3 = M20CandidateSeam.new(); cs3.bind(b3); var w3 = _m20_full(b3, cs3); var l3 = _m20_harness_bind(w3, _m20_slots([c3, c3, c3, c3, c3]))
	var r3 = l3.activate_slot(0, Vector2(-2.0, 11.5), 6.0); cs3.mode = "unrelated_loss"; cs3.loss_color = c3; cs3.loss_index = t3U
	_m19_drive_to_arrival(r3.agent)
	cs3.mode = "normal"
	_check(l3.get_last_outcome() == CompleteClearingLoop.Outcome.CANDIDATE_ROLLBACK and w3["candidates"].get_candidates(c3, null).has(t3T) and w3["candidates"].get_candidates(c3, null).has(t3U), "V09.7: unrelated same-color candidate loss -> both restored")
	l3 = null; _m20_teardown(w3)
	# healthy clear preserves unrelated different-color bucket.
	var b4 = _m19_open_board_active(22, 22, [Vector2(9, 11), Vector2(9, 13)]); var t4: int = b4.get_cell_index(9, 11); var c4: int = b4.get_color_id(t4); var cO: int = b4.get_color_id(b4.get_cell_index(9, 13))
	var w4 = _m20_full(b4); var l4 = _m20_loop(w4, _m20_slots([c4, c4, c4, c4, c4]))
	var oB: Array = w4["candidates"].get_candidates(cO, null)
	_check(_m20_activate_and_arrive(l4, 0, Vector2(-2.0, 11.5)).success, "V09.7: healthy clear")
	_check_eq(w4["candidates"].get_candidates(cO, null), oB, "V09.7: unrelated different-color bucket preserved")
	l4 = null; _m20_teardown(w4)
	# identity swap -> ROLLBACK_FAILED.
	var b5 = _m19_open_board_active(22, 22, [Vector2(9, 11), Vector2(11, 11), Vector2(13, 11)]); var t5T: int = b5.get_cell_index(9, 11); var t5U: int = b5.get_cell_index(11, 11); var t5V: int = b5.get_cell_index(13, 11); var c5: int = b5.get_color_id(t5T)
	var rs5 = M20ReservationSeam.new(); rs5.bind(b5); var w5 = _m20_full(b5, null, rs5); var l5 = _m20_harness_bind(w5, _m20_slots([c5, c5, c5, c5, c5]))
	var r5T = l5.activate_slot(0, Vector2(-2.0, 11.5), 6.0); var r5U = l5.activate_slot(1, Vector2(26.0, 11.5), 6.0)
	rs5.mode = "identity_swap"; rs5.swap_from_owner = r5U.owner_id; rs5.swap_from_target = t5U; rs5.swap_to_target = t5V
	_m19_drive_to_arrival(r5T.agent)
	_check_eq(l5.get_last_outcome(), CompleteClearingLoop.Outcome.ROLLBACK_FAILED, "V09.7: identity swap cannot pass exact owner-map proof -> ROLLBACK_FAILED")
	rs5.mode = "normal"; l5 = null; _m20_teardown(w5)
	# reset during candidate/reservation phase.
	# reset during candidate phase.
	var bpc = _m19_open_board_active(22, 22, [Vector2(11, 11)]); var tpc: int = bpc.get_cell_index(11, 11); var cpc: int = bpc.get_color_id(tpc)
	var csp = M20CandidateSeam.new(); csp.bind(bpc)
	var wpc = _m20_full(bpc, csp); var lpc = _m20_harness_bind(wpc, _m20_slots([cpc, cpc, cpc, cpc, cpc]))
	var rpc = lpc.activate_slot(0, Vector2(-2.0, 11.5), 6.0)
	csp.sync_hook = func(_i): lpc.reset()
	_m19_drive_to_arrival(rpc.agent)
	csp.sync_hook = Callable()
	_check(lpc.get_last_outcome() == CompleteClearingLoop.Outcome.RESET_ABORTED and bpc.get_cell_state(tpc) == BoardState.CellState.ACTIVE, "V09.7: reset during candidate phase -> RESET_ABORTED, target ACTIVE")
	lpc = null; _m20_teardown(wpc)
	# reset during reservation phase.
	var bpr = _m19_open_board_active(22, 22, [Vector2(11, 11)]); var tpr: int = bpr.get_cell_index(11, 11); var cpr: int = bpr.get_color_id(tpr)
	var rsp = M20ReservationSeam.new(); rsp.bind(bpr)
	var wpr = _m20_full(bpr, null, rsp); var lpr = _m20_harness_bind(wpr, _m20_slots([cpr, cpr, cpr, cpr, cpr]))
	var rpr = lpr.activate_slot(0, Vector2(-2.0, 11.5), 6.0)
	rsp.resolve_hook = func(_t, _o): lpr.reset()
	_m19_drive_to_arrival(rpr.agent)
	rsp.resolve_hook = Callable()
	_check(lpr.get_last_outcome() == CompleteClearingLoop.Outcome.RESET_ABORTED and bpr.get_cell_state(tpr) == BoardState.CellState.ACTIVE, "V09.7: reset during reservation phase -> RESET_ABORTED, target ACTIVE")
	lpr = null; _m20_teardown(wpr)
	# missing-current-pair reset does not fabricate/delete unrelated reservation.
	var b6 = _m19_open_board_active(22, 22, [Vector2(11, 11), Vector2(15, 11)])
	var w6 = _m19_wire_real(b6)
	var r6 = w6["dispatcher"].dispatch(b6.get_color_id(b6.get_cell_index(11, 11)), Vector2(-2.0, 11.5), 6.0)
	w6["reservations"].release(r6.target_index, r6.owner_id)
	var u6: int = b6.get_cell_index(15, 11); w6["reservations"].reserve(u6, 6006)
	w6["dispatcher"].reset()
	_check(w6["reservations"].get_owner(u6) == 6006 and w6["reservations"].get_reservation_count() == 1, "V09.7: missing-pair reset keeps unrelated, fabricates none")
	_m19_teardown(w6)
	# healthy pair-narrow + foreign + same-board owner replacement survive.
	var b7 = _m19_open_board_active(22, 22, [Vector2(11, 11), Vector2(15, 11)])
	var w7 = _m19_wire_real(b7)
	var r7 = w7["dispatcher"].dispatch(b7.get_color_id(b7.get_cell_index(11, 11)), Vector2(-2.0, 11.5), 6.0)
	w7["reservations"].reserve(b7.get_cell_index(15, 11), 7007)
	w7["dispatcher"].reset()
	_check(w7["reservations"].get_owner(r7.target_index) == -1 and w7["reservations"].get_owner(b7.get_cell_index(15, 11)) == 7007, "V09.7: pair-narrow reset releases pair, preserves unrelated")
	_m19_teardown(w7)
	var b8 = _m19_open_board_active(22, 22, [Vector2(11, 11), Vector2(15, 11)])
	var w8 = _m19_wire_real(b8)
	var r8 = w8["dispatcher"].dispatch(b8.get_color_id(b8.get_cell_index(11, 11)), Vector2(-2.0, 11.5), 6.0)
	w8["reservations"].release(r8.target_index, r8.owner_id)
	var v8: int = b8.get_cell_index(15, 11); w8["reservations"].reserve(v8, r8.owner_id)
	w8["dispatcher"].reset()
	_check_eq(w8["reservations"].get_owner(v8), r8.owner_id, "V09.7: same-board owner replacement V<->O survives reset")
	_m19_teardown(w8)
	# post-dispatch generation barrier + reset re-entry safe + owner monotonic.
	var wb = _m20_full_reset_access(_m19_open_board_active(22, 22, [Vector2(11, 11), Vector2(13, 11)]))
	var lb = _m20_loop(wb, _m20_slots([wb["board"].get_color_id(wb["board"].get_cell_index(11, 11)), 0, 0, 0, 0]))
	var ob: int = wb["dispatcher"].peek_next_owner_id()
	wb["select_access"].hook = func(): lb.reset()
	_check_eq(lb.activate_slot(0, Vector2(-2.0, 11.5), 6.0).failure_reason, DispatchResult.FailureReason.RESETTING, "V09.7: post-dispatch generation barrier -> RESETTING")
	wb["select_access"].hook = Callable()
	lb.reset(); lb.reset() # re-entry safe
	_check(wb["dispatcher"].peek_next_owner_id() >= ob + 1, "V09.7: owner ids monotonic; reset re-entry safe")
	lb = null; _m20_teardown(wb)

# ==================== M20-C001 V10 closure-only exact-evidence reconciliation ===
# Narrow reconciliation groups G-V09-01..05 (evidence table in CLAUDE_LOG_V10.md).
# Validation-only against locked V07 production. Frame-only cases live in
# tests/m20_v10_lifecycle_smoke.gd.

func _run_m20_v10_closure_reconciliation_tests() -> void:
	print("==== M20-C001 V10 closure reconciliation ====")
	_v10_g01_claim_reset_usability()
	_v10_g02_activation_serialization()
	_v10_g03_failed_preflight_matrix()
	_v10_g04_ledger_identity()
	_v10_g05_rollback_prestate()
	print("  M20 V10 closure reconciliation complete")

# --- G-V09-01: claim/reset ACTUAL usability (second real gameplay op) ---------
func _v10_g01_claim_reset_usability() -> void:
	print("---- V10 G-V09-01: claim/reset actual usability ----")
	# Two reachable same-color targets A (9,11) and B (13,11).
	var board = _m19_open_board_active(22, 22, [Vector2(9, 11), Vector2(13, 11)])
	var color: int = board.get_color_id(board.get_cell_index(9, 11))
	var w = _m20_full(board)
	var diag := func(_o, _t, _c, _a): pass
	w["dispatcher"].assignment_arrived.connect(diag) # diagnostic BEFORE first bind
	var l1 = CompleteClearingLoop.new()
	_check(l1.bind(board, _m20_slots([color, color, color, color, color]), w["candidates"], w["reservations"], w["dispatcher"]), "G01: first loop binds")
	var rA = _m20_activate_and_arrive(l1, 0, Vector2(-2.0, 11.5))
	_check(rA.success and l1.get_cleared_count() == 1, "G01: first loop clears A")
	# Snapshot with NO active work, then reset.
	var cells_snap = _snapshot_cell_states(board)
	var rmap_snap := _v09_rmap(w)
	var rcount_snap: int = w["reservations"].get_reservation_count()
	l1.reset()
	_check(_cell_states_equal(board, cells_snap), "G01: owner reset preserves BoardState snapshot")
	_check(_v09_dict_eq(_v09_rmap(w), rmap_snap) and w["reservations"].get_reservation_count() == rcount_snap, "G01: owner reset preserves exact reservation map/count")
	# Second different loop still cannot bind (claim held by first loop, alive).
	var l2 = CompleteClearingLoop.new()
	_check(not l2.bind(board, _m20_slots([color, color, color, color, color]), w["candidates"], w["reservations"], w["dispatcher"]), "G01: second different loop still cannot bind")
	# ORIGINAL first loop performs a SECOND real activation -> clears B.
	var rB = _m20_activate_and_arrive(l1, 1, Vector2(26.0, 11.5))
	_check(rB.success and rB.target_index == board.get_cell_index(13, 11), "G01: first loop second real activation selects B")
	_check_eq(l1.get_cleared_count(), 2, "G01: first loop cleared_count incremented exactly once more (B)")
	_check_eq(l2.get_cleared_count(), 0, "G01: second loop zero attribution")
	_check_eq(board.get_cell_state(board.get_cell_index(13, 11)), BoardState.CellState.CLEARED, "G01: B actually CLEARED")
	w["dispatcher"].assignment_arrived.disconnect(diag)
	l1 = null; l2 = null
	print("  G01: owner-GC claim release proven in tests/m20_v09/v10 lifecycle smokes")
	_m20_teardown(w)

# --- G-V09-02: activation serialization exactness -----------------------------
func _v10_g02_activation_serialization() -> void:
	print("---- V10 G-V09-02: activation serialization exactness ----")
	# 2A nested activation from the preflight coherence callback.
	var b = _m19_open_board_active(22, 22, [Vector2(11, 11), Vector2(13, 11)])
	var color: int = b.get_color_id(b.get_cell_index(11, 11))
	var seam = M20CandidateSeam.new(); seam.bind(b)
	var w = _m20_full(b, seam)
	var loop = _m20_harness_bind(w, _m20_slots([color, color, 0, 0, 0]))
	var before_owner: int = w["dispatcher"].peek_next_owner_id()
	var nested := [&""]
	seam.coherence_hook = func(): nested[0] = loop.activate_slot(1, Vector2(26.0, 11.5)).failure_reason
	var outer = loop.activate_slot(0, Vector2(-2.0, 11.5), 6.0)
	_check_eq(nested[0], DispatchResult.FailureReason.REENTRANT, "G02.A: nested activation REENTRANT")
	_check(outer.success, "G02.A: outer healthy activation succeeds")
	_check_eq(w["dispatcher"].get_active_count(), 1, "G02.A: active count == 1")
	_check_eq(w["reservations"].get_reservation_count(), 1, "G02.A: reservation count == 1")
	_check_eq(w["dispatcher"].peek_next_owner_id(), before_owner + 1, "G02.A: next owner id == before + 1 (nested consumed no token)")
	seam.coherence_hook = Callable()
	loop = null; _m20_teardown(w)

	# 2B activation DURING arrival drain (_draining true) via the candidate hook.
	var b2 = _m19_open_board_active(22, 22, [Vector2(9, 11), Vector2(13, 11)])
	var c2: int = b2.get_color_id(b2.get_cell_index(9, 11))
	var seam2 = M20CandidateSeam.new(); seam2.bind(b2)
	var w2 = _m20_full(b2, seam2)
	var l2 = _m20_harness_bind(w2, _m20_slots([c2, c2, 0, 0, 0]))
	var rA = l2.activate_slot(0, Vector2(-2.0, 11.5), 6.0)
	var owner_before2: int = w2["dispatcher"].peek_next_owner_id()
	var active_before2: int = w2["dispatcher"].get_active_count()
	var res_before2: int = w2["reservations"].get_reservation_count()
	var inner := [&""]
	seam2.sync_hook = func(_i): inner[0] = l2.activate_slot(1, Vector2(26.0, 11.5)).failure_reason
	_m19_drive_to_arrival(rA.agent) # A commits; _draining true when the hook runs
	_check_eq(inner[0], DispatchResult.FailureReason.REENTRANT, "G02.B: activation during arrival drain -> REENTRANT")
	_check_eq(w2["dispatcher"].peek_next_owner_id(), owner_before2, "G02.B: inner request consumed no owner token")
	_check(l2.get_cleared_count() == 1, "G02.B: A transaction completes normally")
	_check(w2["dispatcher"].get_active_count() <= active_before2, "G02.B: inner created no extra active agent")
	seam2.sync_hook = Callable()
	# After drain: a later ordinary activation remains usable (clears B).
	var rB = _m20_activate_and_arrive(l2, 1, Vector2(26.0, 11.5))
	_check(rB.success and l2.get_cleared_count() == 2, "G02.B: later ordinary activation usable after drain")
	l2 = null; _m20_teardown(w2)

# --- G-V09-03: exact failed-preflight matrix ----------------------------------
func _v10_g03_failed_preflight_matrix() -> void:
	print("---- V10 G-V09-03: exact failed-preflight matrix ----")
	# missing reservation.
	var wm = _m20_full(_m19_open_board_active(22, 22, [Vector2(11, 11), Vector2(13, 11)]))
	var am = _v09_arrived_sentinel(wm)
	wm["reservations"].release_for_owner(am["owner"])
	am["loop"]._on_assignment_arrived(am["owner"], am["target"], am["color"], am["agent"])
	_check_eq(am["loop"].get_last_outcome(), CompleteClearingLoop.Outcome.PREFLIGHT_REJECTED, "G03 missing reservation: rejected")
	_check(am["loop"].get_cleared_count() == 0 and wm["board"].get_cell_state(am["target"]) == BoardState.CellState.ACTIVE, "G03 missing reservation: no clear, ACTIVE")
	_check_eq(wm["reservations"].get_owner(am["sentinel"]), 9090, "G03 missing reservation: sentinel intact")
	am["loop"] = null; _m20_teardown(wm)

	# candidate rebind(foreign): pair + assignment + raw candidate remain.
	var wf = _m20_full(_m19_open_board_active(22, 22, [Vector2(11, 11), Vector2(13, 11)]))
	var af = _v09_arrived_sentinel(wf)
	wf["candidates"].rebind(_m19_open_board_active(22, 22, [Vector2(2, 2)]))
	af["loop"]._on_assignment_arrived(af["owner"], af["target"], af["color"], af["agent"])
	_check_eq(af["loop"].get_last_outcome(), CompleteClearingLoop.Outcome.PREFLIGHT_REJECTED, "G03 candidate foreign: rejected")
	_check(wf["reservations"].get_owner(af["target"]) == af["owner"] and wf["reservations"].get_target_for_owner(af["owner"]) == af["target"], "G03 candidate foreign: reservation both directions held")
	_check(wf["dispatcher"].has_owner(af["owner"]) and af["loop"].get_cleared_count() == 0, "G03 candidate foreign: assignment pending, cleared 0")
	_check_eq(wf["reservations"].get_owner(af["sentinel"]), 9090, "G03 candidate foreign: sentinel intact")
	af["loop"] = null; _m20_teardown(wf)

	# candidate rebind(null).
	var wn = _m20_full(_m19_open_board_active(22, 22, [Vector2(11, 11), Vector2(13, 11)]))
	var an = _v09_arrived_sentinel(wn)
	wn["candidates"].rebind(null)
	an["loop"]._on_assignment_arrived(an["owner"], an["target"], an["color"], an["agent"])
	_check_eq(an["loop"].get_last_outcome(), CompleteClearingLoop.Outcome.PREFLIGHT_REJECTED, "G03 candidate null: rejected cleanly")
	_check(wn["reservations"].get_owner(an["target"]) == an["owner"] and wn["dispatcher"].has_owner(an["owner"]), "G03 candidate null: pair+assignment held")
	an["loop"] = null; _m20_teardown(wn)

	# externally CLEARED target.
	var wc = _m20_full(_m19_open_board_active(22, 22, [Vector2(11, 11), Vector2(13, 11)]))
	var ac = _v09_arrived_sentinel(wc)
	wc["board"].set_cell_state(ac["target"], BoardState.CellState.CLEARED)
	ac["loop"]._on_assignment_arrived(ac["owner"], ac["target"], ac["color"], ac["agent"])
	_check_eq(ac["loop"].get_last_outcome(), CompleteClearingLoop.Outcome.PREFLIGHT_REJECTED, "G03 externally CLEARED: rejected")
	_check_eq(ac["loop"].get_cleared_count(), 0, "G03 externally CLEARED: cleared_count unchanged")
	ac["loop"] = null; _m20_teardown(wc)

	# renderer foreign after dispatch / before arrival.
	var wr = _m20_full(_m19_open_board_active(22, 22, [Vector2(11, 11)]), null, null, true)
	var tr: int = wr["board"].get_cell_index(11, 11); var cr: int = wr["board"].get_color_id(tr)
	var lr = _m20_loop(wr, _m20_slots([cr, cr, cr, cr, cr]))
	var rr = lr.activate_slot(0, Vector2(-2.0, 11.5), 6.0)
	wr["renderer"].configure(_m19_open_board_active(22, 22, [Vector2(2, 2)]), PackedStringArray(BoardDebugFixturesM20.PALETTE), Vector2(90, 90))
	_m19_drive_to_arrival(rr.agent)
	_check(wr["board"].get_cell_state(tr) == BoardState.CellState.ACTIVE and lr.get_cleared_count() == 0, "G03 renderer foreign: no clear")
	_check(wr["reservations"].get_owner(tr) == rr.owner_id and wr["reservations"].get_target_for_owner(rr.owner_id) == tr and wr["dispatcher"].has_owner(rr.owner_id), "G03 renderer foreign: pair both directions + assignment held")
	_check(wr["candidates"].get_candidates(cr, null).has(tr), "G03 renderer foreign: raw target candidate present")
	lr = null; root.remove_child(wr["renderer"]); wr["renderer"].free(); wr["renderer"] = null; _m20_teardown(wr)

	# renderer queued after dispatch / before arrival.
	var wq = _m20_full(_m19_open_board_active(22, 22, [Vector2(11, 11)]), null, null, true)
	var tq: int = wq["board"].get_cell_index(11, 11); var cq: int = wq["board"].get_color_id(tq)
	var lq = _m20_loop(wq, _m20_slots([cq, cq, cq, cq, cq]))
	var rq = lq.activate_slot(0, Vector2(-2.0, 11.5), 6.0)
	wq["renderer"].queue_free()
	_m19_drive_to_arrival(rq.agent)
	_check(wq["board"].get_cell_state(tq) == BoardState.CellState.ACTIVE and lq.get_cleared_count() == 0, "G03 renderer queued: no clear")
	_check(wq["reservations"].get_owner(tq) == rq.owner_id and wq["reservations"].get_target_for_owner(rq.owner_id) == tq and wq["dispatcher"].has_owner(rq.owner_id), "G03 renderer queued: pair both directions + assignment held")
	_check(wq["candidates"].get_candidates(cq, null).has(tq), "G03 renderer queued: raw target candidate present")
	if is_instance_valid(wq["renderer"]): root.remove_child(wq["renderer"]); wq["renderer"].free()
	wq["renderer"] = null; lq = null; _m20_teardown(wq)
	print("  G03: truly-freed renderer preservation + failed-preflight cleanup -> tests/m20_v10_lifecycle_smoke.gd")

# --- G-V09-04: exact ledger identity ------------------------------------------
func _v10_g04_ledger_identity() -> void:
	print("---- V10 G-V09-04: exact ledger identity ----")
	# Five-slot exact both-direction identity.
	var b = _m19_open_board_active(24, 24, [Vector2(12, 0), Vector2(12, 1), Vector2(12, 2), Vector2(12, 3), Vector2(12, 4)])
	var pal: Array = []
	for y in range(5): pal.append(b.get_color_id(b.get_cell_index(12, y)))
	var w = _m20_full(b); var loop = _m20_loop(w, _m20_slots(pal))
	var org: Array = [Vector2(-2.0, 0.5), Vector2(26.0, 1.5), Vector2(-2.0, 2.5), Vector2(26.0, 3.5), Vector2(-2.0, 4.5)]
	var res: Array = []
	for i in range(5):
		var r = loop.activate_slot(i, org[i], 6.0)
		if r.success: res.append(r)
	_check_eq(res.size(), 5, "G04 five-slot: five in-flight")
	_m19_drive_to_arrival(res[0].agent)
	_check_eq(w["reservations"].get_owner(res[0].target_index), -1, "G04 five-slot: first get_owner(target) == -1")
	_check_eq(w["reservations"].get_target_for_owner(res[0].owner_id), -1, "G04 five-slot: first get_target_for_owner(owner) == -1")
	var four := true
	for i in range(1, 5):
		if w["reservations"].get_owner(res[i].target_index) != res[i].owner_id or w["reservations"].get_target_for_owner(res[i].owner_id) != res[i].target_index: four = false
	_check(four, "G04 five-slot: remaining four exact both directions")
	for i in range(1, 5): _m19_drive_to_arrival(res[i].agent)
	_check(w["dispatcher"].get_active_count() == 0 and w["reservations"].get_reservation_count() == 0, "G04 five-slot: all finalize, active 0, reservations 0")
	loop = null; _m20_teardown(w)
	# Scale rows.
	for d in [[24, 24, "Easy", 12], [34, 34, "Medium", 15], [44, 44, "Hard", 20], [54, 54, "VeryHard", 25], [59, 59, "59x59", 29], [53, 59, "rect", 20]]:
		var bd = _m19_open_board_active(d[0], d[1], [Vector2(d[3], 10)])
		var expected_target: int = bd.get_cell_index(d[3], 10)
		var wd = _m20_full(bd); var ld = _m20_loop(wd, _m20_slots([bd.get_color_id(expected_target), 0, 0, 0, 0]))
		var before: int = bd.count_cells_by_state(BoardState.CellState.CLEARED)
		var rd = _m20_activate_and_arrive(ld, 0, Vector2(-2.0, 10.5))
		_check(rd.success and rd.target_index == expected_target, "G04 %s: success at expected target" % d[2])
		_check_eq(bd.get_cell_state(expected_target), BoardState.CellState.CLEARED, "G04 %s: expected target CLEARED" % d[2])
		_check_eq(bd.count_cells_by_state(BoardState.CellState.CLEARED), before + 1, "G04 %s: CLEARED delta exactly +1" % d[2])
		ld = null; _m20_teardown(wd)

# --- G-V09-05: detached rollback prestate exact compare -----------------------
func _v10_g05_rollback_prestate() -> void:
	print("---- V10 G-V09-05: detached rollback prestate ----")
	# Candidate mutate-before-false: T + same-color U + different-color V + unrelated reservation.
	var b = _m19_open_board_active(22, 22, [Vector2(9, 11), Vector2(11, 11), Vector2(9, 13)])
	var tT: int = b.get_cell_index(9, 11); var tU: int = b.get_cell_index(11, 11); var tV: int = b.get_cell_index(9, 13)
	var cT: int = b.get_color_id(tT); var cV: int = b.get_color_id(tV)
	var cs = M20CandidateSeam.new(); cs.bind(b)
	var w = _m20_full(b, cs)
	var loop = _m20_harness_bind(w, _m20_slots([cT, cT, cT, cT, cT]))
	var r = loop.activate_slot(0, Vector2(-2.0, 11.5), 6.0) # reserves T
	w["reservations"].reserve(tV, 5050) # unrelated reservation
	# DETACHED prestate snapshot.
	var pre_cells = _snapshot_cell_states(b)
	var pre_Tbucket: Array = w["candidates"].get_candidates(cT, null)
	var pre_Vbucket: Array = w["candidates"].get_candidates(cV, null)
	var pre_rmap := _v09_rmap(w)
	var pre_rcount: int = w["reservations"].get_reservation_count()
	var pre_active: int = w["dispatcher"].get_active_count()
	var pre_has_owner: bool = w["dispatcher"].has_owner(r.owner_id)
	cs.mode = "mutate_false"
	_m19_drive_to_arrival(r.agent)
	cs.mode = "normal"
	var oc = loop.get_last_outcome()
	_check(oc == CompleteClearingLoop.Outcome.CANDIDATE_ROLLBACK or oc == CompleteClearingLoop.Outcome.ROLLBACK_FAILED, "G05 candidate: CANDIDATE_ROLLBACK or ROLLBACK_FAILED")
	if oc == CompleteClearingLoop.Outcome.CANDIDATE_ROLLBACK:
		_check(_cell_states_equal(b, pre_cells), "G05 candidate: BoardState exactly restored")
		_check(w["candidates"].get_candidates(cT, null) == pre_Tbucket, "G05 candidate: target/same-color bucket exactly restored")
		_check(w["candidates"].get_candidates(cV, null) == pre_Vbucket, "G05 candidate: different-color bucket exactly restored")
		_check(_v09_dict_eq(_v09_rmap(w), pre_rmap) and w["reservations"].get_reservation_count() == pre_rcount, "G05 candidate: reservation map/count exactly restored")
		_check(w["dispatcher"].get_active_count() == pre_active and w["dispatcher"].has_owner(r.owner_id) == pre_has_owner, "G05 candidate: dispatcher active/owner identity restored")
	loop = null; _m20_teardown(w)

	# Reservation mutate-before-false: current pair + unrelated pair.
	var b2 = _m19_open_board_active(22, 22, [Vector2(9, 11), Vector2(13, 11)])
	var t2T: int = b2.get_cell_index(9, 11); var u2: int = b2.get_cell_index(13, 11); var c2: int = b2.get_color_id(t2T)
	var rs = M20ReservationSeam.new(); rs.bind(b2)
	var w2 = _m20_full(b2, null, rs)
	var l2 = _m20_harness_bind(w2, _m20_slots([c2, c2, c2, c2, c2]))
	var r2 = l2.activate_slot(0, Vector2(-2.0, 11.5), 6.0) # reserves T for owner
	w2["reservations"].reserve(u2, 6060) # unrelated pair
	var pre_map2 := _v09_rmap(w2)
	var pre_count2: int = w2["reservations"].get_reservation_count()
	var pre_owner_tgt: int = w2["reservations"].get_target_for_owner(r2.owner_id)
	var pre_u_owner: int = w2["reservations"].get_owner(u2)
	var pre_cells2 = _snapshot_cell_states(b2)
	var pre_active2: int = w2["dispatcher"].get_active_count()
	rs.mode = "mutate_false"
	_m19_drive_to_arrival(r2.agent)
	rs.mode = "normal"
	var oc2 = l2.get_last_outcome()
	_check(oc2 == CompleteClearingLoop.Outcome.RESERVATION_ROLLBACK or oc2 == CompleteClearingLoop.Outcome.ROLLBACK_FAILED, "G05 reservation: RESERVATION_ROLLBACK or ROLLBACK_FAILED")
	if oc2 == CompleteClearingLoop.Outcome.RESERVATION_ROLLBACK:
		_check(_v09_dict_eq(_v09_rmap(w2), pre_map2) and w2["reservations"].get_reservation_count() == pre_count2, "G05 reservation: exact target->owner map/count restored")
		_check(w2["reservations"].get_target_for_owner(r2.owner_id) == pre_owner_tgt and w2["reservations"].get_owner(u2) == pre_u_owner, "G05 reservation: owner->target reverse identity restored (both owners)")
